...
JsonConverter::JsonConverter(const QString &inputJsonFilename,
                             const QString &outputSqliteFilename)
    : m_inputJsonFilename(inputJsonFilename),
      m_outputSqliteFilename(outputSqliteFilename) {}
...
bool JsonConverter::process() {
  ...
  qInfo() << "JSON analysis complete. Output SQLite file will be:"
          << m_outputSqliteFilename;
...
void JsonConverter::reportJsonContents(LdDecodeMetaData &metaData) {
  ...
  qInfo() << "First Field First:"
          << (metaData.getIsFirstFieldFirst() ? "Yes" : "No");

  // Comprehensive Video parameters matching README schema
  const LdDecodeMetaData::VideoParameters &videoParams =
      metaData.getVideoParameters();
  qInfo() << "Video Parameters:";
  qInfo() << "  System:"
          << (videoParams.system == PAL
                  ? "PAL"
  ...
  qInfo() << "  Video Sample Rate:"
          << QString::number(videoParams.sampleRate, 'f', 0).toLongLong()
          << "Hz";
  ...
  qInfo() << "  Is Subcarrier Locked:"
          << (videoParams.isSubcarrierLocked ? "Yes" : "No");
  ...
  const LdDecodeMetaData::PcmAudioParameters &audioParams =
      metaData.getPcmAudioParameters();
  if (audioParams.sampleRate > 0) {
    ...
    qInfo() << "  Is Little Endian:"
            << (audioParams.isLittleEndian ? "Yes" : "No");
  } else {
  ...
  for (qint32 fieldNum = 1; fieldNum <= metaData.getNumberOfFields();
       fieldNum++) {
    ...
    const LdDecodeMetaData::ClosedCaption &cc =
        metaData.getFieldClosedCaption(fieldNum);
    if (cc.inUse) fieldsWithClosedCaptions++;

    // Count VITS Metrics
    const LdDecodeMetaData::VitsMetrics &vits =
        metaData.getFieldVitsMetrics(fieldNum);
  ...
  qInfo() << "  Fields with VBI data:" << fieldsWithVbi << "("
          << QString::number(
                 100.0 * fieldsWithVbi / metaData.getNumberOfFields(), 'f', 1)
                 .toDouble()
          << "%)";
  qInfo() << "  Fields with VITC data:" << fieldsWithVitc << "("
          << QString::number(
                 100.0 * fieldsWithVitc / metaData.getNumberOfFields(), 'f', 1)
                 .toDouble()
          << "%)";
  qInfo() << "  Fields with Closed Caption data:" << fieldsWithClosedCaptions
          << "("
          << QString::number(100.0 * fieldsWithClosedCaptions /
  ...
  qInfo() << "  Fields with VITS Metrics:" << fieldsWithVitsMetrics << "("
          << QString::number(
                 100.0 * fieldsWithVitsMetrics / metaData.getNumberOfFields(),
                 'f', 1)
                 .toDouble()
          << "%)";
  if (videoParams.system == NTSC) {
    qInfo() << "  Fields with NTSC data:" << fieldsWithNtsc << "("
            << QString::number(
                   100.0 * fieldsWithNtsc / metaData.getNumberOfFields(), 'f',
                   1)
                   .toDouble()
            << "%)";
  }
  qInfo() << "  Fields with Audio samples:" << fieldsWithAudio << "("
          << QString::number(
                 100.0 * fieldsWithAudio / metaData.getNumberOfFields(), 'f', 1)
                 .toDouble()
          << "%)";
  qInfo() << "  Padded fields (no valid video):" << paddedFields << "("
          << QString::number(
                 100.0 * paddedFields / metaData.getNumberOfFields(), 'f', 1)
                 .toDouble()
          << "%)";
  ...
  qInfo() << "  PCM audio parameters:"
          << (audioParams.sampleRate > 0 ? "1 row" : "0 rows (no audio)");
...
bool JsonConverter::createSchema() {
  ...
  if (!query.exec("CREATE TABLE capture ("
                  ...
                  "    capture_notes TEXT"
                  ");")) {
    qCritical() << "Failed to create capture table:"
                << query.lastError().text();
    return false;
  }

  // Create pcm_audio_parameters table
  if (!query.exec("CREATE TABLE pcm_audio_parameters ("
                  "    capture_id INTEGER PRIMARY KEY"
                  ...
                  "    sample_rate REAL"
                  ");")) {
    qCritical() << "Failed to create pcm_audio_parameters table:"
                << query.lastError().text();
    return false;
  }

  // Create field_record table
  if (!query.exec("CREATE TABLE field_record ("
                  ...
                  ");")) {
    qCritical() << "Failed to create field_record table:"
                << query.lastError().text();
    return false;
  }

  // Create vits_metrics table
  if (!query.exec("CREATE TABLE vits_metrics ("
                  "    capture_id INTEGER NOT NULL,"
                  "    field_id INTEGER NOT NULL,"
                  "    b_psnr REAL,"
                  "    w_snr REAL,"
                  ...
                  "    PRIMARY KEY (capture_id, field_id)"
                  ");")) {
    qCritical() << "Failed to create vits_metrics table:"
                << query.lastError().text();
    return false;
  }

  // Create vbi table
  if (!query.exec("CREATE TABLE vbi ("
                  "    capture_id INTEGER NOT NULL,"
                  "    field_id INTEGER NOT NULL,"
  ...
  if (!query.exec(
          ...
          "        ON DELETE CASCADE"
          ");")) {
    qCritical() << "Failed to create drop_outs table:"
                << query.lastError().text();
    return false;
  }

  // Create vitc table
  if (!query.exec("CREATE TABLE vitc ("
                  "    capture_id INTEGER NOT NULL,"
                  "    field_id INTEGER NOT NULL,"
  ...
  if (!query.exec("CREATE TABLE closed_caption ("
                  "    capture_id INTEGER NOT NULL,"
                  "    field_id INTEGER NOT NULL,"
                  "    data0 INTEGER,"
                  "    data1 INTEGER,"
                  ...
                  "    PRIMARY KEY (capture_id, field_id)"
                  ");")) {
    qCritical() << "Failed to create closed_caption table:"
                << query.lastError().text();
    return false;
  }

  // Create cinemap table (Josalo-III: written by ld-cinemap; empty if source
  // had no cinemap data)
  if (!query.exec("CREATE TABLE cinemap ("
                  "    capture_id INTEGER NOT NULL,"
                  "    field_id INTEGER NOT NULL,"
                  ...
                  "    PRIMARY KEY (capture_id, field_id)"
                  ");")) {
    qCritical() << "Failed to create cinemap table:"
                << query.lastError().text();
    return false;
  }
...
bool JsonConverter::insertData(LdDecodeMetaData &metaData) {
  ...
  try {
    ...
    QString decoder =
        "ld-decode";  // Default, could be determined from metadata if available
    ...
    if (hasCinemapData)
      qInfo() << "Legacy cinemap data detected:" << nFields
              << "field(s) will be scanned for cinemap rows.";
    ...
    query.bindValue(2, videoParams.gitBranch.isEmpty() ? QVariant()
                                                       : videoParams.gitBranch);
    query.bindValue(3, videoParams.gitCommit.isEmpty() ? QVariant()
                                                       : videoParams.gitCommit);
    ...
    query.bindValue(
        18, videoParams.black16bIre);  // Legacy JSON doesn't have
                                       // blanking_16b_ire, use black_16b_ire
    query.bindValue(19, videoParams.tapeFormat.isEmpty()
                            ? QVariant()
                            : videoParams.tapeFormat);

    if (!query.exec()) {
      qCritical() << "Failed to insert capture record:"
                  << query.lastError().text();
      m_database.rollback();
      return false;
    }
    ...
    if (audioParams.sampleRate > 0) {
      ...
      if (!query.exec()) {
        qCritical() << "Failed to insert PCM audio parameters:"
                    << query.lastError().text();
        m_database.rollback();
        return false;
      }
    ...
    for (qint32 fieldNum = 1; fieldNum <= metaData.getNumberOfFields();
      ...
      query.bindValue(1,
                      field.audioSamples > 0 ? field.audioSamples : QVariant());
      query.bindValue(2,
                      field.decodeFaults > 0 ? field.decodeFaults : QVariant());
      ...
      if (!query.exec()) {
        qCritical() << "Failed to insert field record for field" << fieldNum
                    << ":" << query.lastError().text();
        m_database.rollback();
        return false;
      }
      ...
      if (vits.inUse) {
        ...
        if (!query.exec()) {
          qCritical() << "Failed to insert VITS metrics for field" << fieldNum
                      << ":" << query.lastError().text();
          m_database.rollback();
          return false;
        }
      ...
      if (vbi.inUse && vbi.vbiData.size() >= 3) {
        ...
        if (!query.exec()) {
          qCritical() << "Failed to insert VBI data for field" << fieldNum
                      << ":" << query.lastError().text();
          m_database.rollback();
          return false;
        }
      ...
      if (vitc.inUse && vitc.vitcData.size() >= 8) {
        query.prepare(
            "INSERT INTO vitc (capture_id, field_id, vitc0, vitc1, vitc2, "
            "vitc3, vitc4, vitc5, vitc6, vitc7) "
            "VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        query.bindValue(0, fieldId);
        ...
        if (!query.exec()) {
          qCritical() << "Failed to insert VITC data for field" << fieldNum
                      << ":" << query.lastError().text();
          m_database.rollback();
          return false;
        }
      ...
      if (cc.inUse) {
        ...
        if (!query.exec()) {
          qCritical() << "Failed to insert closed caption data for field"
                      << fieldNum << ":" << query.lastError().text();
          m_database.rollback();
          return false;
        }
      }
      ...
      for (qint32 i = 0; i < dropouts.size(); i++) {
        ...
        if (!query.exec()) {
          ...
          if (errorText.contains("UNIQUE constraint failed",
                                 Qt::CaseInsensitive)) {
            qInfo() << "Skipping duplicate dropout in field" << fieldNum
                    << "dropout" << i << ": "
            ...
            qCritical() << "Failed to insert dropout data for field" << fieldNum
                        << "dropout" << i << ":" << errorText;
            m_database.rollback();
            return false;
          }
      ...
      // Insert cinemap data if present for this field (cadenceConfidence
      // dropped)
      if (hasCinemapData) {
        const bool hasRow = field.isEditBoundary || field.cadenceId != -1 ||
        ...
        if (hasRow) {
          ...
          query.bindValue(2, field.cadenceId != -1 ? QVariant(field.cadenceId)
                                                   : QVariant());
          query.bindValue(3, field.cadenceIndexPresumed ? 1 : 0);
          query.bindValue(4, field.pulldownRole.isEmpty() ? QVariant()
                                                          : field.pulldownRole);

          if (!query.exec()) {
            qCritical() << "Failed to insert cinemap data for field" << fieldNum
                        << ":" << query.lastError().text();
            m_database.rollback();
            return false;
          }
      ...
      if (fieldNum % 1000 == 0) {
        qInfo() << "Inserted" << fieldNum << "of"
                << metaData.getNumberOfFields() << "fields...";
      }
    }
