 ...
 * File:        main.cpp
 * Module:      cli
 * Purpose:     Command-line entry point for the ld-json-converter tool
 *
...
int main(int argc, char *argv[]) {
  ...
  QCommandLineOption outputJsonOption(
      QStringList() << "output-sqlite",
      QCoreApplication::translate("main",
                                  "Specify the output SQLite file (default "
                                  "same as input but with .db extension)"),
      QCoreApplication::translate("main", "filename"));
  ...
  if (parser.isSet(inputJsonOption)) {
    ...
    if (positionalArguments.count() == 1) {
      ...
      // Quit with error
      qCritical(
          "You must specify an input JSON file using --input-json or as a "
          "positional argument");
      return -1;
    }
  }
  ...
  if (parser.isSet(outputJsonOption)) {
    ...
    if (inputJsonFilename.endsWith(".json", Qt::CaseInsensitive)) {
      outputSqliteFilename =
          inputJsonFilename.left(inputJsonFilename.length() - 5) + ".db";
    } else {
