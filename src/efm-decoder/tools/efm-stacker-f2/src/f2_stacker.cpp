...
bool F2Stacker::process(const QVector<QString>& inputFilenames,
  ...
  for (int index = 0; index < inputFilenames.size(); index++) {
    ...
    if (!reader->open(inputFilenames[index])) {
      qCritical() << "F2Stacker::process() - Could not open input file"
  ...
  for (int inputFileIdx = 0; inputFileIdx < m_inputFiles.size();
       inputFileIdx++) {
    ...
    if (!readNextValidSection(sourceStates[inputFileIdx])) {
      qCritical() << "F2Stacker::process() - Input file"
                  << inputFilenames[inputFileIdx]
    ...
    qInfo().noquote() << "Input File" << inputFilenames[inputFileIdx]
                      << "- First valid section:"
                      << sourceStates[inputFileIdx]
                             .currentSection.metadata.absoluteSectionTime()
                             .toString()
                      << "- Total sections:"
                      << m_inputFiles[inputFileIdx]->size();
  }

  // Open the output file
  if (!m_outputFile.open(outputFilename)) {
    qCritical() << "F2Stacker::process() - Could not open output file"
                << outputFilename;
    return false;
  }
  ...
  // Process sources in timestamp order.  Each source is read forward once;
  // missing or duplicate sections affect only that source rather than shifting
  // later votes.
  while (true) {
    qint32 address = -1;
    for (int sourceIndex = 0; sourceIndex < sourceStates.size();
         sourceIndex++) {
      if (!sourceStates[sourceIndex].hasCurrentSection) {
        continue;
      }

      qint32 sourceAddress = sourceStates[sourceIndex]
                                 .currentSection.metadata.absoluteSectionTime()
                                 .frames();
    ...
    if (sectionList.isEmpty()) {
      qCritical().noquote()
          << "F2Stacker::process() - No sources available for section"
          << SectionTime(address).toString();
      return false;
    }

    tbcDebugStream().noquote() << "F2Stacker::process() - Stacking section"
                               << SectionTime(address).toString();

    F2Section stackedF2Section =
        stackSections(sectionList, sectionAnalyses, sectionSourceIndexes);
    ...
    if (sectionsStacked % 2500 == 0) {
      qInfo().noquote().nospace()
          << "Processed " << sectionsStacked << " sections, current section "
          << SectionTime(address).toString();
    }
  }
  ...
  qInfo().noquote() << "  Start time:"
                    << SectionTime(stackStartAddress).toString();
  ...
  qInfo().noquote().nospace() << "  Padded frames: " << m_paddedFrames << " ("
                              << m_paddedFrames / 98 << " sections)";
  qInfo().noquote() << "  Flat source sections excluded:" << m_flatSections;
  qInfo().noquote() << "  Divergent source sections excluded:"
                    << m_divergentSections;
  qInfo().noquote() << "  Total frames:"
                    << m_errorFreeFrames + m_errorFrames + m_paddedFrames;
  qInfo().noquote() << "";
  qInfo().noquote() << "  Valid bytes common to all sources:"
                    << m_validValueForByte;
  qInfo().noquote() << "  Valid bytes that differed in value between sources:"
                    << m_usedMostCommonValue;
  qInfo().noquote() << "  Tied valid byte values marked as errors:"
                    << m_tiedValueForByte;
  qInfo().noquote() << "  Invalid byte in all sources:"
                    << m_noValidValueForByte;
  qInfo().noquote() << "";
  qInfo().noquote() << "  Source differences:";
  for (int sourceIndex = 0; sourceIndex < m_sourceDifferences.size();
       sourceIndex++) {
    qInfo().noquote() << "    Source" << sourceIndex
                      << inputFilenames[sourceIndex] << ":"
                      << m_sourceDifferences[sourceIndex];
  }
  qInfo().noquote() << "";
  qInfo().noquote() << "  Source missing sections:";
  for (int sourceIndex = 0; sourceIndex < m_sourceMissingSections.size();
       sourceIndex++) {
    qInfo().noquote() << "    Source" << sourceIndex
                      << inputFilenames[sourceIndex] << ":"
                      << m_sourceMissingSections[sourceIndex];
  ...
  for (int sourceIndex = 0; sourceIndex < sourceStates.size(); sourceIndex++) {
    qInfo().noquote() << "    Source" << sourceIndex
                      << inputFilenames[sourceIndex] << "- Invalid metadata:"
                      << sourceStates[sourceIndex].invalidMetadataSections
                      << "- Duplicates:"
                      << sourceStates[sourceIndex].duplicateSections
                      << "- Replaced duplicates:"
                      << sourceStates[sourceIndex].replacedDuplicateSections
                      << "- Out of order:"
                      << sourceStates[sourceIndex].outOfOrderSections;
  }

  return true;
}

bool F2Stacker::readNextValidSection(SourceState& sourceState,
                                     qint32 minimumAddress) {
  while (sourceState.nextSectionNumber < sourceState.reader->size()) {
    ...
    if (sourceState.lastReadAddress >= 0 &&
        address < sourceState.lastReadAddress) {
      sourceState.outOfOrderSections++;
      if (minimumAddress >= 0 && address <= minimumAddress) {
        continue;
      }
...
F2Stacker::SectionAnalysis F2Stacker::analyzeSection(
    const F2Section& section) const {
  ...
  analysis.isFlat = totalBytes == 0 || byteCounts.size() <= 8 ||
                    repeatedFrames >= 80 ||
...
quint32 F2Stacker::sectionDifference(const F2Section& firstSection,
...
F2Section F2Stacker::stackSections(
    const QVector<F2Section>& f2Sections,
  ...
  if (!gotValidMetadata) {
    for (int sectionIndex = 0; sectionIndex < f2Sections.size();
         sectionIndex++) {
      if (f2Sections[sectionIndex].metadata.isValid()) {
        stackedMetadata = f2Sections[sectionIndex].metadata;
        gotValidMetadata = true;
  ...
  if (!gotValidMetadata) {
    qFatal(
        "F2Stacker::stackSections - No valid metadata found in the input "
        "sections");
  }
  ...
  for (int sectionIndex = 0; sectionIndex < f2Sections.size(); sectionIndex++) {
    if (sectionAnalyses[sectionIndex].isPadding) {
      tbcDebugStream().noquote() << "F2Stacker::stackSections - Section"
                                 << sectionIndex << "is just padding";
    } else if (sectionAnalyses[sectionIndex].isFlat) {
      tbcDebugStream().noquote() << "F2Stacker::stackSections - Section"
                                 << sectionIndex << "is flat data";
      m_flatSections++;
    } else {
  ...
  if (validF2Sections.isEmpty()) {
    ...
    for (int firstIndex = 0; firstIndex < validSectionCount; firstIndex++) {
      ...
      for (int secondIndex = firstIndex + 1; secondIndex < validSectionCount;
           secondIndex++) {
        const quint32 differences = sectionDifference(
            validF2Sections[firstIndex], validF2Sections[secondIndex]);
        sectionDifferences[(firstIndex * validSectionCount) + secondIndex] =
            differences;
        sectionDifferences[(secondIndex * validSectionCount) + firstIndex] =
            differences;
      }
    }
    ...
    for (int seedIndex = 0; seedIndex < validSectionCount; seedIndex++) {
      ...
      for (int candidateIndex = 0; candidateIndex < validSectionCount;
           candidateIndex++) {
        const quint32 differences =
            sectionDifferences[(seedIndex * validSectionCount) +
                               candidateIndex];
        if (differences <= sameSectionDifferenceThreshold) {
          ...
          if (m_previousClusterSourceIndexes.contains(
                  validSourceIndexes[candidateIndex])) {
    ...
    if (!bestClusterIndexes.isEmpty() &&
        bestClusterIndexes.size() < validF2Sections.size()) {
      QVector<F2Section> clusteredF2Sections;
      QVector<SectionAnalysis> clusteredSectionAnalyses;
      QVector<int> clusteredSourceIndexes;

      for (int clusterIndex = 0; clusterIndex < bestClusterIndexes.size();
           clusterIndex++) {
      ...
      m_divergentSections +=
          validF2Sections.size() - clusteredF2Sections.size();
    ...
    for (int frameIndex = 0; frameIndex < 98; frameIndex++) {
      ...
      for (int sectionIndex = 0; sectionIndex < validF2Sections.size();
           sectionIndex++) {
...
F2Frame F2Stacker::stackFrames(QVector<F2Frame>& f2Frames,
  ...
  for (int byteIndex = 0; byteIndex < 32; byteIndex++) {
    ...
    if (validByteCount == 0) {
      ...
      tbcDebugStream()
          << "F2Stacker::stackFrames - No valid byte value for index"
          << byteIndex;
      m_noValidValueForByte++;
    } else {
      ...
      if (allBytesSame) {
        ...
        for (int validIndex = 0; validIndex < validByteCount; validIndex++) {
          validBytesString.append(
              QString("%1 ")
                  .arg(validBytes[validIndex], 2, 16, QChar('0'))
                  .toUpper());
        }
        QString mostCommonByteString =
            QString("%1").arg(mostCommonByte, 2, 16, QChar('0')).toUpper();

        if (tiedMaxCount > 1) {
          tbcDebugStream().noquote()
              << "F2Stacker::stackFrames - Valid byte values tie - marking "
                 "byte as error from"
              << validBytesString;
          stackedFrameData.append(firstValidByte);
          stackedFrameErrorData.append(true);
          ...
          tbcDebugStream().noquote()
              << "F2Stacker::stackFrames - Valid byte values differ - using"
