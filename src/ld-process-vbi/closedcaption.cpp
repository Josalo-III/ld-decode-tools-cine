...
/*!
    ...
    [CTA] "Line 21 Data Services",
   (https://shop.cta.tech/products/line-21-data-services) ANSI/CTA-608-E S-2019,
   April 2008.
*/

// Public method to read CEA-608 Closed Captioning data.
// Return true if CC data was decoded successfully, false otherwise.
bool ClosedCaption::decodeLine(
  ...
  qint32 zcPoint =
      ((videoParameters.white16bIre - videoParameters.black16bIre) / 4) +
  ...
  double x = static_cast<double>(videoParameters.colourBurstEnd) +
             (2.0 * samplesPerBit);
  double xLimit =
      static_cast<double>(videoParameters.fieldWidth) - (17.0 * samplesPerBit);
  double lastOne = x;
  while ((x - lastOne) < (1.5 * samplesPerBit)) {
    if (x >= xLimit) {
      tbcDebugStream()
          << "ClosedCaption::decodeLine(): No start bits found (00)";
      return false;
    }
  ...
  tbcDebugStream()
      << "ClosedCaption::decodeLine(): Found start bit transition at" << x;
  ...
  tbcDebugStream().nospace()
      << "ClosedCaption::decodeLine(): Bytes are: " << byte0 << " ("
      << byte0Parity << ") - " << byte1 << " (" << byte1Parity << ")";

  if (isEvenParity(byte0) && byte0Parity != 1) {
    tbcDebugStream()
        << "ClosedCaption::decodeLine(): First byte failed parity check!";
  } else {
  ...
  if (isEvenParity(byte1) && byte1Parity != 1) {
    tbcDebugStream()
        << "ClosedCaption::decodeLine(): Second byte failed parity check!";
  } else {
