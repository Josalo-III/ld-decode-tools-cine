...
bool FmCode::decodeLine(
  ...
  if (decodeCount != 40) {
    if (decodeCount == 0)
      tbcDebugStream()
          << "FmCode::fmDecoder(): No FM code data found in the field line";
    else
      tbcDebugStream() << "FmCode::fmDecoder(): FM decode failed!  Only got"
                       << decodeCount << "bits";
    return false;
  }

  // Show the 40-bit FM coded data as hexadecimal
  tbcDebugStream() << "FmCode::fmDecoder(): 40-bit FM code is"
                   << QString::number(decodedBytes, 16);
  ...
  tbcDebugStream() << "FmCode::fmDecoder(): receiverClockSyncBits ="
                   << receiverClockSyncBits;
  tbcDebugStream() << "FmCode::fmDecoder(): videoFieldIndicator ="
                   << videoFieldIndicator;
  tbcDebugStream() << "FmCode::fmDecoder(): leadingDataRecognitionBits ="
                   << leadingDataRecognitionBits;
  tbcDebugStream() << "FmCode::fmDecoder(): dataValue =" << dataValue;
  tbcDebugStream() << "FmCode::fmDecoder(): dataParityBit =" << dataParityBit;
  tbcDebugStream() << "FmCode::fmDecoder(): trailingDataRecognitionBits ="
                   << trailingDataRecognitionBits;

  // Sanity check the data
  if (receiverClockSyncBits != 3 || leadingDataRecognitionBits != 114 ||
      trailingDataRecognitionBits != 13) {
