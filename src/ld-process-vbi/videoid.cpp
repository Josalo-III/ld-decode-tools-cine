 ...
 * SPDX-FileCopyrightText: 2023 Phillip Blucas
 */
...
bool VideoID::decodeLine(
  ...
  qint32 zcPoint =
      ((videoParameters.white16bIre - videoParameters.black16bIre) * 35 / 100) +
  ...
  double samplesPerBit =
      static_cast<double>(videoParameters.fieldWidth) * 16 / 455;
  ...
  double xLimit =
      static_cast<double>(videoParameters.fieldWidth) - 22.0 * samplesPerBit;
  ...
  tbcDebugStream() << "VideoID::decodeLine(): 20-bit code is"
                   << QStringLiteral("%1").arg(codeWord, 20, 2,
                                               QLatin1Char('0'));
  ...
  tbcDebugStream() << "VideoID::decodeLine(): word0 ="
                   << QStringLiteral("%1").arg(word0, 2, 2, QLatin1Char('0'));
  tbcDebugStream() << "VideoID::decodeLine(): word1 ="
                   << QStringLiteral("%1").arg(word1, 4, 2, QLatin1Char('0'));
  tbcDebugStream() << "VideoID::decodeLine(): word2 ="
                   << QStringLiteral("%1").arg(word2, 8, 2, QLatin1Char('0'));
  tbcDebugStream() << "VideoID::decodeLine(): crcc  ="
                   << QStringLiteral("%1").arg(crcc, 6, 2, QLatin1Char('0'));
  ...
  if (crc != crcc) {
    tbcDebugStream() << "VideoID::decodeLine(): Invalid CRC"
                     << QStringLiteral("%1").arg(crc, 6, 2, QLatin1Char('0'));
    return false;
  }
