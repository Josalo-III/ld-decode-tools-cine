 ...
 * File:        main.cpp
 * Module:      cli
 * Purpose:     Command-line entry point for the ld-process-vbi tool
 *
...
int main(int argc, char *argv[]) {
  ...
  QCommandLineOption inputMetadataOption(
      ...
      QCoreApplication::translate(
          "main", "Specify the input metadata file (default input.db)"),
  ...
  QCommandLineOption outputMetadataOption(
      ...
      QCoreApplication::translate(
          "main", "Specify the output metadata file (default same as input)"),
      QCoreApplication::translate("main", "filename"));
  parser.addOption(outputMetadataOption);

  // Option to disable metadata back-up (-n)
  QCommandLineOption showNoBackupOption(
      QStringList() << "n" << "nobackup",
      QCoreApplication::translate(
          "main", "Do not create a backup of the input metadata"));
  ...
  qInfo().nospace().noquote()
      << "Reading metadata from " << inputMetadataFilename;
  ...
  if (inputMetadataFilename == outputMetadataFilename && !noBackup) {
    qInfo().nospace().noquote()
        << "Backing up metadata to " << inputMetadataFilename << ".bup";
    if (!QFile::copy(inputMetadataFilename, inputMetadataFilename + ".bup")) {
      qCritical()
          << "Unable to back-up input metadata file - back-up already exists?";
      return 1;
    }
  }
  ...
  DecoderPool decoderPool(inputFilename, outputMetadataFilename, maxThreads,
                          metaData);
