...
bool BiphaseCode::decodeLines(
    const SourceVideo::Data& line16Data, const SourceVideo::Data& line17Data,
...
// Decode one of the three biphase code lines, writing the result into
// fieldMetadata. Return true if decoding was successful, false otherwise.
bool BiphaseCode::decodeLine(
    qint32 lineIndex, const SourceVideo::Data& lineData,
    const LdDecodeMetaData::VideoParameters& videoParameters,
    LdDecodeMetaData::Field& fieldMetadata) {
  ...
  fieldMetadata.vbi.vbiData[lineIndex] =
      manchesterDecoder(lineData, zcPoint, videoParameters);

  return (fieldMetadata.vbi.vbiData[lineIndex] != 0);
}

// Private method to read a 24-bit biphase coded signal (manchester code) from a
// field line
qint32 BiphaseCode::manchesterDecoder(
    const SourceVideo::Data& lineData, qint32 zcPoint,
  ...
  if (x < manchesterData.size()) {
    ...
    // Find the rest of the transitions based on the expected clock rate of 2us
    // per cell window
  ...
  if (decodeCount != 24) {
    ...
    if (decodeCount != 0) {
      tbcDebugStream()
          << "BiphaseCode::manchesterDecoder(): Manchester decode failed!  Got"
          << decodeCount << "bits, expected 24";
      // -1 is a good choice to indicate a parse error because it does not
      // conflict with 0 (black line) or >0 (successfully parsed data)
      // differentiating between parse errors and black lines is useful because
      // if parse errors are known, they can be autofixed by studying the
      // surrounding picture number cadence
      result = -1;
    }
  }
