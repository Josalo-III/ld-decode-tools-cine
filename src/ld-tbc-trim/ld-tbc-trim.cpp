/*
 * File:        ld-tbc-trim.cpp
 * Module:      cli
 * Purpose:     Command-line entry point for the TBC trimmer and decomposer
 *
...
static void printHelp() {
  printf(
      ...
      "  ld-tbc-trim -cav <frame_number> [-l <length>] <input.tbc> "
      "<output.tbc>\n"
      "  ld-tbc-trim -clv <hh:mm:ss:ff> [-l <length>] <input.tbc> "
      "<output.tbc>\n"
      "\n"
      ...
      "  -clv <hh:mm:ss:ff>   Start at CLV timecode "
      "(hours:minutes:seconds:frame)\n"
      "  -l <length>          Number of frames (default: to end of source)\n"
      "\n"
      "Decompose options:\n"
      "  --decompose          Split into one file pair per contiguous mappable "
      "span;\n"
      "                       padded and lead-in/out frames are excluded\n"
      "  --decompose-edits    Split at ld-cinemap edit boundaries; padded "
      "frames\n"
      "                       are included within segments; lead-in/out "
      "excluded\n"
      "\n"
...
static bool parseClvTimecode(const QString& str,
...
int main(int argc, char* argv[]) {
  ...
  parser.addOption(QCommandLineOption(QStringList() << "s" << "seq",
                                      "Sequential start frame number (1-based)",
                                      "start"));
  parser.addOption(QCommandLineOption("cav", "CAV picture number", "frame"));
  parser.addOption(
      QCommandLineOption("clv", "CLV timecode (hh:mm:ss:ff)", "timecode"));
  ...
  parser.addPositionalArgument("output",
                               "Output TBC file (trim) or stem (decompose)");
  ...
  if (doDecompose && doDecomposeEdits) {
    qCritical(
        "Error: --decompose and --decompose-edits are mutually exclusive.");
    return -1;
  }
  ...
  if (isDecomposeMode && selectorCount > 0) {
    qCritical(
        "Error: --decompose and --decompose-edits cannot be combined with -s, "
        "-cav, or -clv.");
    return -1;
  }
  ...
  if (isDecomposeMode && parser.isSet("l")) {
    qCritical(
        "Error: --decompose and --decompose-edits cannot be combined with -l.");
    return -1;
  }
  ...
  if (!srcMd.read(inputDbPath)) {
    qCritical("Error: Cannot read input metadata from %s",
              qPrintable(inputDbPath));
    return -1;
  }
  ...
  if (doDecomposeEdits && !vp.isCinemapped) {
    qCritical(
        "Error: --decompose-edits requires ld-cinemap to have been run on this "
        "source first.");
    return -1;
  }

  // --- Build frame list ---
  const QVector<TbcTrimUtils::FrameInfo> frames =
      TbcTrimUtils::buildFrameList(&srcMd);
  ...
  if (isDecomposeMode) {
    ...
    if (doDecompose) {
      ...
      QString error;
      segments =
          TbcTrimUtils::buildDecomposeEditSegments(&srcMd, frames, error);
      if (!error.isEmpty()) {
    ...
    const TbcTrimUtils::DecomposeStats stats = TbcTrimUtils::writeSegments(
        &srcMd, frames, segments, outputArg, QFileInfo(inputTbcPath),
    ...
    if (stats.overlapFrameCount == 0) {
      ...
      qInfo(
          "%d segment(s) created with %d frame(s) repeated across %d "
          "segment(s).",
          stats.segmentCount, stats.overlapFrameCount, stats.overlapSegments);
    }
  ...
  if (bySeq) {
    ...
    if (!parseClvTimecode(parser.value("clv"), tc)) {
      qCritical(
          "Error: Invalid CLV timecode format. Use hh:mm:ss:ff (e.g., "
          "00:42:15:05)");
      return -1;
    }
  ...
  if (length == -1) {
    ...
    qInfo(
        "No length specified - trimming from frame %d to end of source (%d "
        "frames).",
        startIdx + 1, length);
  }
  ...
  if (startIdx + length > inTotalFrames) {
    ...
    qWarning(
        "Warning: Requested %d frames from frame %d but only %d are available "
        "- "
        "clamping to %d frames.",
        length, startIdx + 1, clamped, clamped);
    length = clamped;
  }

  qInfo() << "Trimming frames" << (startIdx + 1) << "to" << (startIdx + length)
          << "inclusive.";

  // --- Write output metadata ---
  LdDecodeMetaData* trimmedMd =
      TbcTrimUtils::sliceMetadata(&srcMd, frames, startIdx, length);
  if (!trimmedMd) {
  ...
  // --- Write TBC video ---
  const auto writeFrames =
      TbcTrimUtils::buildWriteFrames(frames, startIdx, length);
  ...
  if (!TbcStreamWriter::write(writeFrames, &srcMd, QFileInfo(inputTbcPath),
                              QFileInfo(outputTbcPath), config)) {
