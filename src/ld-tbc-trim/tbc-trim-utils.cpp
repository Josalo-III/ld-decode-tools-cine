...
QVector<FrameInfo> buildFrameList(LdDecodeMetaData* md) {
  ...
  for (int fi = 0; fi < nFrames; ++fi) {
    ...
    if (info.firstFieldNumber <= 0 || info.secondFieldNumber <= 0) {
      ...
      auto vbi = vbiDecoder.decodeFrame(vbi1[0], vbi1[1], vbi1[2], vbi2[0],
                                        vbi2[1], vbi2[2]);
...
LdDecodeMetaData* sliceMetadata(LdDecodeMetaData* src,
  ...
  if (startFrame < 0 || length <= 0 || startFrame + length > frames.size())
    return nullptr;
...
bool findStartByCav(LdDecodeMetaData* md, const QVector<FrameInfo>& frames,
  ...
  for (int i = 0; i < nFrames; ++i) {
    ...
    if (fi.isPadded || fi.firstFieldNumber <= 0 || fi.secondFieldNumber <= 0)
      continue;
...
bool findStartByClv(LdDecodeMetaData* md, const QVector<FrameInfo>& frames,
  ...
  for (int i = 0; i < nFrames; ++i) {
    ...
    if (current.hours == tc.hours && current.minutes == tc.minutes &&
...
QVector<TbcStreamWriter::WriteFrame> buildWriteFrames(
  ...
  for (int i = 0; i < length; ++i) {
    ...
    writeFrames.push_back(
        {fi.firstFieldNumber, fi.secondFieldNumber, fi.isPadded});
  }
...
QVector<Segment> buildDecomposeSegments(const QVector<FrameInfo>& frames) {
  ...
  while (i < n) {
    ...
    if (seg.length > 0) segments.append(seg);
  }
...
QVector<Segment> buildDecomposeEditSegments(LdDecodeMetaData* md,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    else if (f2.cinemap.inUse && f2.cinemap.isEditBoundary &&
             !f2.isFirstField) {
      cuts.append({i, true});
    }
  }
  ...
  if (cuts.isEmpty()) {
    outError =
        "No edit boundary frames found. Run ld-cinemap to establish edit "
        "points before using --decompose-edits.";
    return segments;
  }
  ...
  // Find the last non-lead-in-out, non-padded frame as the implicit end
  // (exclusive)
  int contentEnd = n - 1;
  while (contentEnd >= 0 &&
  ...
  for (int s = 0; s < starts.size(); ++s) {
    ...
    int segEnd =
        (s + 1 < starts.size()) ? starts[s + 1].frameIndex : contentEnd;
...
DecomposeStats writeSegments(LdDecodeMetaData* src,
  ...
  for (int s = 0; s < segments.size(); ++s) {
    ...
    const QString segName =
        QString("%1_%2.tbc").arg(outputStem).arg(s + 1, 3, 10, QChar('0'));
    ...
    // Write metadata
    LdDecodeMetaData* segMd =
        sliceMetadata(src, frames, seg.startFrame, seg.length);
    if (!segMd) {
      qWarning() << "Failed to slice metadata for segment" << (s + 1)
                 << "- skipping";
      continue;
    }
    ...
    const auto writeFrames =
        buildWriteFrames(frames, seg.startFrame, seg.length);
    if (!TbcStreamWriter::write(writeFrames, src, inputFile, QFileInfo(segName),
                                config)) {
