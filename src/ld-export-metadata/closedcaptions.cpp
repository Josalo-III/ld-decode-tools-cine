...
// Extract any available CC data and output it in Scenarist Closed Caption
// format (SCC) V1.0 Protocol description:
...
bool writeClosedCaptions(LdDecodeMetaData &metaData, const QString &fileName) {
  ...
  if (!file.open(QFile::WriteOnly | QFile::Text)) {
    tbcDebug(
        QStringLiteral("writeClosedCaptions: Could not open file for output"));
    return false;
  }
  ...
  for (qint32 fieldIndex = 1;
       fieldIndex <= videoParameters.numberOfSequentialFields; fieldIndex++) {
    // Get the CC data bytes from the field
    qint32 data0 =
        sanityCheckData(metaData.getFieldClosedCaption(fieldIndex).data0);
    qint32 data1 =
        sanityCheckData(metaData.getFieldClosedCaption(fieldIndex).data1);
    ...
    if (data0 == -1 || data1 == -1) {
      ...
      if (data0 > 0 || data1 > 0) {
        if (captionInProgress == false) {
          ...
          QString timeStamp =
              generateTimeStamp(fieldIndex, videoParameters.system);
          stream << "\n\n" << timeStamp << "\t";
          debugCaption =
              "writeClosedCaptions(): Caption data at " + timeStamp + " : [";
