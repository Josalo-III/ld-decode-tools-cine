...
DropOutCorrect::DropOutCorrect(QAtomicInt &_abort,
                               CorrectorPool &_correctorPool, QObject *parent)
...
void DropOutCorrect::run() {
  ...
  while (!abort) {
    ...
    if (!correctorPool.getInputFrame(
            frameNumber, firstFieldSeqNo, firstSourceField, firstFieldMetadata,
    ...
    tbcDebugStream().nospace()
        << "DropOutCorrect::process(): Frame #" << frameNumber
        << " - There are " << totalAvailableSources
        << " sources available of which " << availableSourcesForFrame.size()
        << " contain the required frame";
    ...
    if (firstFieldMetadata[0].dropOuts.empty() &&
        secondFieldMetadata[0].dropOuts.empty()) {
      ...
      // Perform correction...
      tbcDebugStream().nospace()
          << "DropOutCorrect::process(): Correcting fields ["
          << firstFieldSeqNo[0] << "/" << secondFieldSeqNo[0] << "] containing "
          << firstFieldMetadata[0].dropOuts.size() +
                 secondFieldMetadata[0].dropOuts.size()
          << " drop-outs";
      ...
      for (qint32 i = 0; i < availableSourcesForFrame.size(); i++) {
        ...
        if (firstFieldMetadata[currentSource].dropOuts.size() > 0)
          firstFieldDropouts[currentSource] =
              setDropOutLocations(populateDropoutsVector(
                  firstFieldMetadata[currentSource], overCorrect));
      }
      ...
      for (qint32 i = 0; i < availableSourcesForFrame.size(); i++) {
        ...
        if (secondFieldMetadata[currentSource].dropOuts.size() > 0)
          secondFieldDropouts[currentSource] =
              setDropOutLocations(populateDropoutsVector(
                  secondFieldMetadata[currentSource], overCorrect));
      }

      // Correct the first field
      correctField(firstFieldDropouts, secondFieldDropouts, firstFieldData,
                   secondFieldData, true, intraField, availableSourcesForFrame,
                   sourceFrameQuality, statistics);

      // Correct the second field
      correctField(secondFieldDropouts, firstFieldDropouts, secondFieldData,
                   firstFieldData, false, intraField, availableSourcesForFrame,
                   sourceFrameQuality, statistics);
    }

    // Return the processed fields
    correctorPool.setOutputFrame(
        frameNumber, firstFieldData[0], secondFieldData[0], firstFieldSeqNo[0],
        secondFieldSeqNo[0], statistics.sameSourceConcealment,
        statistics.multiSourceConcealment, statistics.multiSourceCorrection,
        statistics.totalReplacementDistance);
  }
}

// Correct dropouts within one field
void DropOutCorrect::correctField(
    ...
    const QVector<SourceVideo::Data> &otherFieldData, bool thisFieldIsFirst,
    bool intraField, const QVector<qint32> &availableSourcesForFrame,
    const QVector<double> &sourceFrameQuality, Statistics &statistics) {
  for (qint32 dropoutIndex = 0; dropoutIndex < thisFieldDropouts[0].size();
       dropoutIndex++) {
    ...
    if (thisFieldDropouts[0][dropoutIndex].location == Location::colourBurst) {
      replacement = findReplacementLine(
          thisFieldDropouts, otherFieldDropouts, dropoutIndex, thisFieldIsFirst,
          true, true, intraField, availableSourcesForFrame, sourceFrameQuality);
    }
    ...
    if (thisFieldDropouts[0][dropoutIndex].location == Location::visibleLine) {
      ...
      replacement = findReplacementLine(
          thisFieldDropouts, otherFieldDropouts, dropoutIndex, thisFieldIsFirst,
          false, false, intraField, availableSourcesForFrame,
          sourceFrameQuality);
      chromaReplacement = findReplacementLine(
          thisFieldDropouts, otherFieldDropouts, dropoutIndex, thisFieldIsFirst,
          true, false, intraField, availableSourcesForFrame,
          sourceFrameQuality);
    }

    // Correct the data
    correctDropOut(thisFieldDropouts[0][dropoutIndex], replacement,
                   chromaReplacement, thisFieldData, otherFieldData,
                   statistics);
  }
}

// Populate the dropouts vector
QVector<DropOutCorrect::DropOutLocation> DropOutCorrect::populateDropoutsVector(
    LdDecodeMetaData::Field field, bool overCorrect) {
  QVector<DropOutLocation> fieldDropOuts;

  for (qint32 dropOutIndex = 0; dropOutIndex < field.dropOuts.size();
       dropOutIndex++) {
    ...
    if (dropOutLocation.fieldLine < 1 ||
        dropOutLocation.fieldLine > videoParameters[0].fieldHeight) {
      continue;
    }
    ...
    if (overCorrect) {
      ...
      // damaged discs where drop-outs can 'slope' in and out fooling
      // ld-decode's detection mechanisms
      ...
      if (dropOutLocation.endx <
          videoParameters[0].fieldWidth - overCorrectionDots)
        dropOutLocation.endx += overCorrectionDots;
      else
        dropOutLocation.endx = videoParameters[0].fieldWidth;
    }
...
QVector<DropOutCorrect::DropOutLocation> DropOutCorrect::setDropOutLocations(
    QVector<DropOutCorrect::DropOutLocation> dropOuts) {
  ...
  do {
    ...
    for (qint32 index = 0; index < noOfDropOuts; index++) {
      ...
      // Note: Here we use the colour burst end as the active video start (to
      // prevent a case where the drop out begins between the colour burst level
      // end and active video start and would go undetected)
      else if (dropOuts[index].startx > videoParameters[0].colourBurstEnd &&
               dropOuts[index].startx <= videoParameters[0].activeVideoEnd) {
...
// Find a replacement line to take replacement data from.  This method looks
// both up and down the field for the nearest replacement line that doesn't
// contain a drop-out itself (to prevent copying bad data over bad data).
DropOutCorrect::Replacement DropOutCorrect::findReplacementLine(
    const QVector<QVector<DropOutLocation>> &thisFieldDropouts,
    const QVector<QVector<DropOutLocation>> &otherFieldDropouts,
  ...
  if (!matchChromaPhase) {
    ...
    stepAmount = 1;
    otherFieldOffset = -1;
  } else if (videoParameters[0].system == PAL ||
             videoParameters[0].system == PAL_M) {
  ...
  for (qint32 i = 0; i < availableSourcesForFrame.size(); i++) {
    ...
    findPotentialReplacementLine(thisFieldDropouts, dropOutIndex,
                                 ...
                                 currentSource, sourceFrameQuality, candidates);

    // Look down the field for a replacement
    findPotentialReplacementLine(
        thisFieldDropouts, dropOutIndex, thisFieldDropouts, true, stepAmount,
        stepAmount, currentSource, sourceFrameQuality, candidates);
    ...
    if (!isColourBurst && !intraField) {
      ...
      // Look up the field for a replacement
      findPotentialReplacementLine(thisFieldDropouts, dropOutIndex,
                                   otherFieldDropouts, false, otherFieldOffset,
                                   -stepAmount, currentSource,
                                   sourceFrameQuality, candidates);

      // Look down the field for a replacement
      findPotentialReplacementLine(
          thisFieldDropouts, dropOutIndex, otherFieldDropouts, false,
          otherFieldOffset + stepAmount, stepAmount, currentSource,
          sourceFrameQuality, candidates);
    }
  }

  tbcDebugStream() << (isColourBurst ? "Colourburst" : "Visible video")
                   << "dropout on line"
                   << thisFieldDropouts[0][dropOutIndex].fieldLine << "of"
                   << (thisFieldIsFirst ? "first" : "second") << "field";
  ...
  if (!candidates.empty()) {
    ...
    for (const Replacement &candidate : candidates) {
      ...
      // The first field (in a .tbc, for both PAL and NTSC) contains the top
      // frame line.
      const qint32 dropoutFrameLine =
          (2 * thisFieldDropouts[0][dropOutIndex].fieldLine) +
          (thisFieldIsFirst ? 0 : 1);
      const qint32 sourceFrameLine =
          (2 * candidate.fieldLine) + (candidate.isSameField
                                           ? (thisFieldIsFirst ? 0 : 1)
                                           : (thisFieldIsFirst ? 1 : 0));

      const qint32 distance = qAbs(dropoutFrameLine - sourceFrameLine);
      tbcDebugStream() << (candidate.isSameField ? "This" : "Other")
                       << "field replacement candidate for line"
                       << thisFieldDropouts[0][dropOutIndex].fieldLine
                       << "is line" << candidate.fieldLine << "distance"
                       << distance << "of source" << candidate.sourceNumber
      ...
      // Choose the candidate if the distance is the same, but the quality is
      // better
      else if (distance == replacement.distance &&
               candidate.quality > replacement.quality) {
        replacement = candidate;
        replacement.distance = distance;
      }
    }
  ...
  if (replacement.fieldLine != -1) {
    tbcDebugStream() << "Selected replacement is"
                     << (replacement.isSameField ? "same" : "other")
                     << "field, line" << replacement.fieldLine << "of source"
                     << replacement.sourceNumber
                     << (matchChromaPhase ? "(chroma phase matched)"
                                          : "(whole signal)")
                     << "with a quality of" << replacement.quality;
  } else {
    tbcDebugStream() << "No viable replacement selected for"
                     << thisFieldDropouts[0][dropOutIndex].fieldLine;
  }
...
// Given a dropout, scan through a source field for the nearest replacement line
// that doesn't have overlapping dropouts. Adds a Replacement to candidates if
// one was found.
void DropOutCorrect::findPotentialReplacementLine(
  ...
  // Calculate the start source line, applying sourceOffset to find a line with
  // the right chroma phase
  ...
  if ((sourceLine - 1) < videoParameters[sourceNo].firstActiveFieldLine ||
    ...
    tbcDebugStream() << "Line" << sourceLine
                     << "is not in active range - ignoring";
    return;
  }

  // Hunt for a replacement, stopping at the last full line before the half-line
  // between fields
  while ((sourceLine - 1) >= videoParameters[sourceNo].firstActiveFieldLine &&
    ...
    for (qint32 sourceIndex = 0; sourceIndex < sourceDropouts[sourceNo].size();
         sourceIndex++) {
      if (sourceDropouts[sourceNo][sourceIndex].fieldLine == sourceLine &&
          (targetDropouts[0][targetIndex].endx -
           sourceDropouts[sourceNo][sourceIndex].startx) >= 0 &&
          (sourceDropouts[sourceNo][sourceIndex].endx -
           targetDropouts[0][targetIndex].startx) >= 0) {
...
void DropOutCorrect::correctDropOut(
    const DropOutLocation &dropOut, const Replacement &replacement,
    const Replacement &chromaReplacement,
    QVector<SourceVideo::Data> &thisFieldData,
    const QVector<SourceVideo::Data> &otherFieldData, Statistics &statistics) {
  if (replacement.fieldLine == -1) {
  ...
  quint16 *targetLine =
      thisFieldData[0].data() +
      ((dropOut.fieldLine - 1) * videoParameters[0].fieldWidth);

  // Choose whole signal or just chroma replacement
  // Don't use chroma if the source of the replacement is > 0 and coming from
  // the same line in another source
  if ((chromaReplacement.fieldLine == -1) ||
    ...
    tbcDebugStream() << "Whole signal replacement - Source is fieldline"
                     << replacement.fieldLine << "from source"
                     << replacement.sourceNumber;
    ...
    tbcDebugStream() << "Luma replacement - Source is fieldline"
                     << replacement.fieldLine << "from source"
                     << replacement.sourceNumber;
    tbcDebugStream() << "Chroma replacement - Source is fieldline"
                     << chromaReplacement.fieldLine << "from source"
                     << chromaReplacement.sourceNumber;
    ...
    // Extract HF from chromaReplacement (by extracting LF, then subtracting
    // from the original)
    const quint16 *chromaLine =
        (chromaReplacement.isSameField
             ? thisFieldData[replacement.sourceNumber].data()
             : otherFieldData[replacement.sourceNumber].data()) +
  ...
  // Update statistics
  if (replacement.sourceNumber == 0)
    statistics.sameSourceConcealment++;
  else {
    if (replacement.distance == 0)
      statistics.multiSourceCorrection++;
    else
      statistics.multiSourceConcealment++;
  }
