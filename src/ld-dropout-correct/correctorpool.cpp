...
CorrectorPool::CorrectorPool(QString _outputFilename,
    ...
    : QObject(parent),
      ...
      reverse(_reverse),
      intraField(_intraField),
      overCorrect(_overCorrect),
      abort(false),
      ldDecodeMetaData(_ldDecodeMetaData),
      sourceVideos(_sourceVideos) {}
...
bool CorrectorPool::process() {
  ...
  // If there is a leading field in the TBC which is out of field order, we need
  // to copy it to ensure the metadata files match up
  ...
  if (sourceVideos.size() > 1) {
    qInfo() << "Performing multi-source correction... Scanning source videos "
               "for VBI frame number ranges...";
    // Get the VBI frame range for all sources
    if (!setMinAndMaxVbiFrames()) {
      qInfo() << "It was not possible to determine the VBI frame number range "
                 "for the source video - cannot continue!";
      return false;
    }
  }
  ...
  qInfo() << "Using" << maxThreads << "threads to process"
          << ldDecodeMetaData[0]->getNumberOfFrames() << "frames";
  ...
  qInfo() << "Dropout correction complete -" << lastFrameNumber << "frames in"
          << totalSecs << "seconds (" << lastFrameNumber / totalSecs << "FPS )";
...
// reached.
bool CorrectorPool::getInputFrame(
    ...
    QVector<LdDecodeMetaData::VideoParameters>& videoParameters, bool& _reverse,
  ...
  tbcDebugStream().nospace()
      << "CorrectorPool::getInputFrame(): Processing sequential frame number #"
  ...
  if (numberOfSources > 1)
    currentVbiFrame = convertSequentialFrameNumberToVbi(frameNumber, 0);
  for (qint32 sourceNo = 0; sourceNo < numberOfSources; sourceNo++) {
    ...
    if (sourceNo == 0) {
      ...
      firstFieldNumber[sourceNo] =
          ldDecodeMetaData[sourceNo]->getFirstFieldNumber(frameNumber);
      secondFieldNumber[sourceNo] =
          ldDecodeMetaData[sourceNo]->getSecondFieldNumber(frameNumber);

      // Determine the frame quality (currently this is based on frame average
      // black SNR)
      double firstFrameSnr = ldDecodeMetaData[sourceNo]
                                 ->getField(firstFieldNumber[sourceNo])
                                 .vitsMetrics.bPSNR;
      double secondFrameSnr = ldDecodeMetaData[sourceNo]
                                  ->getField(secondFieldNumber[sourceNo])
                                  .vitsMetrics.bPSNR;
      sourceFrameQuality[sourceNo] = (firstFrameSnr + secondFrameSnr) / 2.0;

      tbcDebugStream().nospace()
          << "CorrectorPool::getInputFrame(): Source #0 fields are "
          << firstFieldNumber[sourceNo] << "/" << secondFieldNumber[sourceNo]
          << " (quality is " << sourceFrameQuality[sourceNo] << ")";
    } else if (currentVbiFrame >= sourceMinimumVbiFrame[sourceNo] &&
      ...
      qint32 currentSourceFrameNumber =
          convertVbiFrameNumberToSequential(currentVbiFrame, sourceNo);
      firstFieldNumber[sourceNo] =
          ldDecodeMetaData[sourceNo]->getFirstFieldNumber(
              currentSourceFrameNumber);
      secondFieldNumber[sourceNo] =
          ldDecodeMetaData[sourceNo]->getSecondFieldNumber(
              currentSourceFrameNumber);
      ...
      tbcDebugStream().nospace()
          << "CorrectorPool::getInputFrame(): Source #" << sourceNo
          << " has VBI frame number " << currentVbiFrame << " and fields "
          << firstFieldNumber[sourceNo] << "/" << secondFieldNumber[sourceNo]
          << " (quality is " << sourceFrameQuality[sourceNo] << ")";
    } else {
      tbcDebugStream().nospace()
          << "CorrectorPool::getInputFrame(): Source #" << sourceNo
          << " does not contain a usable frame";
    }
    ...
    if (firstFieldNumber[sourceNo] != -1 && secondFieldNumber[sourceNo] != -1) {
      // Fetch the input data (get the fields in TBC sequence order to save
      // seeking)
      if (firstFieldNumber[sourceNo] < secondFieldNumber[sourceNo]) {
        firstFieldVideoData[sourceNo] =
            sourceVideos[sourceNo]->getVideoField(firstFieldNumber[sourceNo]);
        secondFieldVideoData[sourceNo] =
            sourceVideos[sourceNo]->getVideoField(secondFieldNumber[sourceNo]);
      } else {
        secondFieldVideoData[sourceNo] =
            sourceVideos[sourceNo]->getVideoField(secondFieldNumber[sourceNo]);
        firstFieldVideoData[sourceNo] =
      ...
      firstFieldMetadata[sourceNo] =
          ldDecodeMetaData[sourceNo]->getField(firstFieldNumber[sourceNo]);
      secondFieldMetadata[sourceNo] =
          ldDecodeMetaData[sourceNo]->getField(secondFieldNumber[sourceNo]);
      videoParameters[sourceNo] =
          ldDecodeMetaData[sourceNo]->getVideoParameters();
    }
  }

  // Figure out which of the available sources can be used to correct the
  // current frame
...
bool CorrectorPool::setOutputFrame(
    qint32 frameNumber, SourceVideo::Data firstTargetFieldData,
    SourceVideo::Data secondTargetFieldData, qint32 firstFieldSeqNo,
    qint32 secondFieldSeqNo, qint32 sameSourceConcealment,
    qint32 multiSourceConcealment, qint32 multiSourceCorrection,
  ...
  while (pendingOutputFrames.contains(outputFrameNumber)) {
    const OutputFrame& outputFrame =
        pendingOutputFrames.value(outputFrameNumber);

    // Save the frame data to the output file (with the fields in the correct
    // order)
    ...
    if (outputFrame.firstFieldSeqNo < outputFrame.secondFieldSeqNo) {
      ...
      if (!writeOutputField(outputFrame.secondTargetFieldData))
        writeFail = true;
    } else {
    ...
    if (outputFrame.sameSourceConcealment + outputFrame.multiSourceConcealment +
            outputFrame.multiSourceCorrection >
        0) {
      avgReplacementDistance =
          static_cast<double>(outputFrame.totalReplacementDistance) /
          static_cast<double>(outputFrame.sameSourceConcealment +
                              outputFrame.multiSourceConcealment +
                              outputFrame.multiSourceCorrection);
      tbcDebugStream().nospace()
          << "Processed frame " << outputFrameNumber << " with "
          << outputFrame.sameSourceConcealment +
                 outputFrame.multiSourceConcealment +
                 outputFrame.multiSourceCorrection
          << " changes (" << outputFrame.sameSourceConcealment << ", "
          << outputFrame.multiSourceConcealment << ", "
          << outputFrame.multiSourceCorrection << " - avg dist. "
          << avgReplacementDistance << ")";
    } else {
      tbcDebugStream() << "Processed frame" << outputFrameNumber
                       << "- no dropouts";
    }
...
bool CorrectorPool::setMinAndMaxVbiFrames() {
  ...
  for (qint32 sourceNumber = 0; sourceNumber < numberOfSources;
       sourceNumber++) {
    ...
    for (qint32 seqFrame = 1;
         seqFrame <= ldDecodeMetaData[sourceNumber]->getNumberOfFrames();
         seqFrame++) {
      // Get the VBI data and then decode
      auto vbi1 =
          ldDecodeMetaData[sourceNumber]
              ->getFieldVbi(
                  ldDecodeMetaData[sourceNumber]->getFirstFieldNumber(seqFrame))
              .vbiData;
      auto vbi2 = ldDecodeMetaData[sourceNumber]
                      ->getFieldVbi(
                          ldDecodeMetaData[sourceNumber]->getSecondFieldNumber(
                              seqFrame))
                      .vbiData;
      VbiDecoder::Vbi vbi = vbiDecoder.decodeFrame(vbi1[0], vbi1[1], vbi1[2],
      ...
      if (vbi.clvHr != -1 && vbi.clvMin != -1 && vbi.clvSec != -1 &&
        ...
        qint32 cvFrameNumber =
            ldDecodeMetaData[sourceNumber]->convertClvTimecodeToFrameNumber(
                timecode);
    ...
    tbcDebugStream() << "CorrectorPool::setMinAndMaxVbiFrames(): Got"
                     << cavCount << "CAV picture codes and" << clvCount
                     << "CLV timecodes";

    // If the metadata has no picture numbers or time-codes, we cannot use the
    // source
    if (cavCount == 0 && clvCount == 0) {
      tbcDebugStream() << "CorrectorPool::setMinAndMaxVbiFrames(): Source does "
                          "not seem to contain valid CAV picture numbers or "
                          "CLV time-codes - cannot process";
      return false;
    }
    ...
    if (cavCount > clvCount) {
      ...
      tbcDebugStream() << "CorrectorPool::setMinAndMaxVbiFrames(): Got"
                       << cavCount
                       << "valid CAV picture numbers - source disc type is CAV";
      qInfo().nospace() << "Source #" << sourceNumber
                        << " has a disc type of CAV (uses VBI frame numbers)";
      ...
      tbcDebugStream() << "CorrectorPool::setMinAndMaxVbiFrames(): Got"
                       << clvCount
                       << "valid CLV picture numbers - source disc type is CLV";
      qInfo().nospace() << "Source #" << sourceNumber
                        << " has a disc type of CLV (uses VBI time codes)";
    ...
    qInfo().nospace() << "Source #" << sourceNumber
                      << " has a VBI frame number range of "
                      << sourceMinimumVbiFrame[sourceNumber] << " to "
...
// Method to convert the first source sequential frame number to a VBI frame
// number
qint32 CorrectorPool::convertSequentialFrameNumberToVbi(
    qint32 sequentialFrameNumber, qint32 sourceNumber) {
...
qint32 CorrectorPool::convertVbiFrameNumberToSequential(qint32 vbiFrameNumber,
                                                        qint32 sourceNumber) {
...
// Method that returns a vector of the sources that contain data for the
// required VBI frame number
QVector<qint32> CorrectorPool::getAvailableSourcesForFrame(
  ...
  for (qint32 sourceNo = 0; sourceNo < sourceVideos.size(); sourceNo++) {
    if (vbiFrameNumber >= sourceMinimumVbiFrame[sourceNo] &&
      ...
      qint32 firstFieldNumber = ldDecodeMetaData[sourceNo]->getFirstFieldNumber(
          convertVbiFrameNumberToSequential(vbiFrameNumber, sourceNo));
...
bool CorrectorPool::writeOutputField(const SourceVideo::Data& fieldData) {
  return targetVideo.write(reinterpret_cast<const char*>(fieldData.data()),
                           2 * fieldData.size());
}
