 ...
 * File:        main.cpp
 * Module:      cli
 * Purpose:     Command-line entry point for the ld-export-decode-metadata tool
 *
...
int main(int argc, char *argv[]) {
  ...
  QCommandLineOption inputSqliteOption(
      QStringList() << "input-sqlite",
  ...
  QCommandLineOption outputJsonOption(
      QStringList() << "output-json",
      QCoreApplication::translate("main",
                                  "Specify the output JSON file (default same "
                                  "as input but with -export.json extension)"),
      QCoreApplication::translate("main", "filename"));
  ...
  if (parser.isSet(inputSqliteOption)) {
    ...
    if (positionalArguments.count() == 1) {
      ...
      // Quit with error
      qCritical(
          "You must specify an input SQLite file using --input-sqlite or as a "
          "positional argument");
      return -1;
    }
  }
  ...
  if (parser.isSet(outputJsonOption)) {
    ...
    if (inputSqliteFilename.endsWith(".db", Qt::CaseInsensitive)) {
      outputJsonFilename =
          inputSqliteFilename.left(inputSqliteFilename.length() - 3) +
          ".export.json";
    } else {
