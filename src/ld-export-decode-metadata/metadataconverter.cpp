...
MetadataConverter::MetadataConverter(const QString &inputSqliteFilename,
                                     const QString &outputJsonFilename)
    : m_inputSqliteFilename(inputSqliteFilename),
      m_outputJsonFilename(outputJsonFilename) {}
...
bool MetadataConverter::process() {
  ...
  if (!ExportMetaData::parseVideoSystemName(
          ldDecodeMetaData.getVideoSystemDescription(),
          out_VideoParameters.system)) {
    qCritical() << "Unsupported video system - cannot continue";
    return false;
  }

  convertVideoParamters(ldDecodeMetaData.getVideoParameters(),
                        out_VideoParameters);
  exportMetaData.setVideoParameters(out_VideoParameters);

  try {
    ExportMetaData::PcmAudioParameters out_PcmAudioParameters;
    convertPcmAudioParamters(ldDecodeMetaData.getPcmAudioParameters(),
                             out_PcmAudioParameters);
    exportMetaData.setPcmAudioParameters(out_PcmAudioParameters);
  } catch (...) {
    qInfo()
        << "No valid PcmAudioParameters, will not export audio information.";
  }

  for (qint32 fieldNum = 1; fieldNum <= ldDecodeMetaData.getNumberOfFields();
       fieldNum++) {
...
void MetadataConverter::convertVideoParamters(
    const LdDecodeMetaData::VideoParameters &in_VideoParameters,
    ExportMetaData::VideoParameters &out_VideoParameters) {
  out_VideoParameters.isSubcarrierLocked =
      in_VideoParameters.isSubcarrierLocked;
...
void MetadataConverter::convertPcmAudioParamters(
...
void MetadataConverter::convertField(const LdDecodeMetaData::Field &in_field,
  ...
  if (out_field.vbi.inUse)
    std::copy(in_field.vbi.vbiData.begin(), in_field.vbi.vbiData.end(),
              out_field.vbi.vbiData.begin());
  ...
  if (out_field.vitc.inUse)
    std::copy(in_field.vitc.vitcData.begin(), in_field.vitc.vitcData.end(),
              out_field.vitc.vitcData.begin());
  ...
  for (int i = 0; i < in_field.dropOuts.size(); i++) {
    out_field.dropOuts.append(in_field.dropOuts.startx(i),
                              in_field.dropOuts.endx(i),
                              in_field.dropOuts.fieldLine(i));
  }
}
