...
DataConverter::DataConverter(QString inputFileNameParam,
                             ...
                             bool isRIFFParam, QObject *parent)
    : QObject(parent) {
...
bool DataConverter::openInputFile(void) {
  ...
  if (inputFileName.isEmpty()) {
    ...
    tbcDebugStream() << "DataConverter::openInputFile(): Input file is"
                     << inputFileName << "and is" << inputFileHandle->size()
                     << "bytes in length";
  }
...
bool DataConverter::openOutputFile(void) {
  ...
  if (outputFileName.isEmpty()) {
    ...
    if (!outputFileHandle->open(QIODevice::WriteOnly)) {
      ...
      tbcDebugStream() << "DataConverter::openOutputFile(): Could not open"
                       << outputFileName << "as output file";
      return false;
    }
    tbcDebugStream() << "DataConverter::openOutputFile(): Output file is"
                     << outputFileName;
  }
...
void DataConverter::packFile(void) {
  ...
  while (!isComplete) {
    ...
    do {
      receivedBytes = inputFileHandle->read(
          reinterpret_cast<char *>(inputBuffer.data() + totalReceivedBytes),
    ...
    if (totalReceivedBytes != 0) {
      ...
      tbcDebugStream() << "DataConverter::packFile(): Got" << totalReceivedBytes
      ...
      for (qint32 wordPointer = 0; wordPointer < (totalReceivedBytes / 2);
           wordPointer += 4) {
        ...
        outputBuffer[outputBufferPointer + 0] =
            static_cast<char>((word0 & 0x03FC) >> 2);
        outputBuffer[outputBufferPointer + 1] = static_cast<char>(
            ((word0 & 0x0003) << 6) + ((word1 & 0x03F0) >> 4));
        outputBuffer[outputBufferPointer + 2] = static_cast<char>(
            ((word1 & 0x000F) << 4) + ((word2 & 0x03C0) >> 6));
        outputBuffer[outputBufferPointer + 3] = static_cast<char>(
            ((word2 & 0x003F) << 2) + ((word3 & 0x0300) >> 8));
        outputBuffer[outputBufferPointer + 4] =
            static_cast<char>(word3 & 0x00FF);
      ...
      if (!outputFileHandle->write(
              reinterpret_cast<char *>(outputBuffer.data()),
              outputBuffer.size())) {
        // File write failed
        qCritical("Could not write to output file!");
      }
      tbcDebugStream() << "DataConverter::packFile(): Wrote"
                       << outputBuffer.size() << "bytes to output file";
    } else {
      // Input file is empty
      tbcDebugStream()
          << "DataConverter::packFile(): Got zero bytes from input file";
      isComplete = true;
    }
  }
}
...
void DataConverter::unpackFile(void) {
  ...
  if (isRIFF) {
    ...
    QString riffHex =
        "52494646FFFFFFFF57415645666D74201000000001000100409C000088580100020010"
        "004C4953541A000000494E464F495346540E0000004C61766635382E32392E31303000"
        "64617461FFFFFFFF";
  ...
  while (!isComplete) {
    ...
    if (totalReceivedBytes != 0) {
      ...
      tbcDebugStream() << "DataConverter::unpackFile(): Got"
                       << totalReceivedBytes << "bytes from input file";
      ...
      for (qint32 bytePointer = 0; bytePointer < totalReceivedBytes;
           bytePointer += 5) {
        ...
        // Use multiplication instead of left-shift to avoid implicit conversion
        // issues
        ...
        output[outputBufferPointer + 0] =
            static_cast<qint16>((word0 - 512) * 64);
        output[outputBufferPointer + 1] =
            static_cast<qint16>((word1 - 512) * 64);
        output[outputBufferPointer + 2] =
            static_cast<qint16>((word2 - 512) * 64);
        output[outputBufferPointer + 3] =
            static_cast<qint16>((word3 - 512) * 64);
      ...
      tbcDebugStream() << "DataConverter::unpackFile(): Wrote"
                       << outputBuffer.size() << "bytes to output file";
    } else {
      // Input file is empty
      tbcDebugStream()
          << "DataConverter::unpackFile(): Got zero bytes from input file";
      isComplete = true;
    }
  }
}
