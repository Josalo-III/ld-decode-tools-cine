...
void Comb::FrameBuffer::buildConstrainedYWitness()
{
    ...
    const bool patchDiagEnabled = configuration.debugPhaseLegs;
    ...
    if (patchDiagEnabled) {
         ...
         */
        if (!witnessWindowDumped && configuration.witnessDumpLine0 >= 0) {
            int l0 = std::clamp(configuration.witnessDumpLine0, first, last - 1);
            int l1 = std::clamp(configuration.witnessDumpLine1, first, last - 1);
            int x0 = std::clamp(configuration.witnessDumpX0, 0, width - 1);
            int x1 = std::clamp(configuration.witnessDumpX1, 0, width - 1);
