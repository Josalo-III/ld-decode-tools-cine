 ...
 * File:        discmap.cpp
 * Module:      model
 * Purpose:     Frame-level disc model built from TBC metadata and VBI
 *
...
DiscMap::DiscMap(const QFileInfo& metadataFileInfo,
                 const bool reverseFieldOrder, const bool noStrict)
  ...
  if (!ldDecodeMetaData->read(metadataFileInfo.filePath())) {
    tbcDebugStream() << "Cannot load metadata from"
                     << metadataFileInfo.filePath();
    m_tbcValid = false;
    return;
  }
  ...
  if (m_numberOfFrames < 2) {
    tbcDebugStream() << "Metadata contains only" << m_numberOfFrames
                     << "frames - too small";
    m_tbcValid = false;
    return;
  }
  ...
  if (m_numberOfFrames > 108000) {
    // 108000 frames = 60 minutes at 30fps (NTSC) — beyond any real LaserDisc
    // side
    tbcDebugStream() << "Metadata contains" << m_numberOfFrames
                     << "frames - too big";
    m_tbcValid = false;
    return;
  }
  ...
  for (qint32 frameNumber = 0; frameNumber < m_numberOfFrames; frameNumber++) {
    ...
    m_frames[frameNumber].firstField(
        ldDecodeMetaData->getFirstFieldNumber(frameNumber + 1));
    m_frames[frameNumber].secondField(
        ldDecodeMetaData->getSecondFieldNumber(frameNumber + 1));

    auto vbi1 = ldDecodeMetaData
                    ->getFieldVbi(
                        ldDecodeMetaData->getFirstFieldNumber(frameNumber + 1))
                    .vbiData;
    auto vbi2 = ldDecodeMetaData
                    ->getFieldVbi(
                        ldDecodeMetaData->getSecondFieldNumber(frameNumber + 1))
                    .vbiData;
    vbiData[frameNumber] = vbiDecoder.decodeFrame(vbi1[0], vbi1[1], vbi1[2],
  ...
  if (ldDecodeMetaData->getVideoParameters().system == PAL)
    m_isDiscPal = true;
  else if (ldDecodeMetaData->getVideoParameters().system == NTSC)
    m_isDiscPal = false;
  else {
    tbcDebugStream() << "Input TBC video system" << m_videoSystemDescription
                     << "is not supported";
    qCritical("Video system must be PAL or NTSC");
  }

  if (m_isDiscPal) {
    // PAL: 44100 Hz / 50 fields/s = 882 samples/field; 882 * 4 bytes (16-bit
    // stereo) = 3528
    m_audioFieldByteLength = 3528;
    m_audioFieldSampleLength = 882;
  } else {
    // NTSC: 44100 Hz / 60 fields/s = 735 samples/field; 736 * 4 bytes = 2944
  ...
  tbcDebugStream() << "Checking first" << framesToCheck
                   << "sequential frames for disc CAV/CLV type determination";
  ...
  for (qint32 frameNumber = 0; frameNumber < framesToCheck; frameNumber++) {
    ...
    if (vbiData[frameNumber].clvHr != -1 && vbiData[frameNumber].clvMin != -1 &&
        vbiData[frameNumber].clvSec != -1 &&
        vbiData[frameNumber].clvPicNo != -1)
      clvCount++;
  }

  if (cavCount == 0 && clvCount == 0) {
    tbcDebugStream() << "Source does not seem to contain valid CAV picture "
                        "numbers or CLV time-codes - cannot map";
    m_tbcValid = false;
    return;
  }
  ...
  if (cavCount > clvCount) {
    m_isDiscCav = true;
    m_discType = "CAV";
    tbcDebugStream() << "Got" << cavCount << "valid CAV picture numbers from"
                     << framesToCheck << "frames - source disc type is CAV";
  } else {
    m_isDiscCav = false;
    m_discType = "CLV";
    tbcDebugStream() << "Got" << clvCount << "valid CLV picture numbers from"
                     << framesToCheck << "frames - source disc type is CLV";
  }

  if (m_isDiscCav)
    tbcDebugStream() << "Storing VBI CAV picture numbers as frame numbers";
  else
    tbcDebugStream() << "Converting VBI CLV timecodes into frame numbers";
  ...
  for (qint32 frameNumber = 0; frameNumber < m_numberOfFrames; frameNumber++) {
    if (!m_isDiscCav) {
      ...
      m_frames[frameNumber].vbiFrameNumber(
          ldDecodeMetaData->convertClvTimecodeToFrameNumber(clvTimecode));
      ...
      if (!m_isDiscPal) {
        ...
        if (isNtscAmendment2ClvFrameNumber(
                m_frames[frameNumber].vbiFrameNumber() - iecOffset)) {
  ...
  if (!m_isDiscPal && m_isDiscCav) {
    ...
    for (qint32 frameNumber = 0; frameNumber < m_numberOfFrames;
         frameNumber++) {
      bool isPulldown = false;

      if (m_frames[frameNumber].vbiFrameNumber() == -1 &&
          !m_frames[frameNumber].isLeadInOrOut()) {
        qint32 lastPhase2 = -1;
        if (frameNumber > 0)
          lastPhase2 = ldDecodeMetaData
                           ->getField(ldDecodeMetaData->getSecondFieldNumber(
                               frameNumber))
                           .fieldPhaseID;

        qint32 currentPhase1 =
            ldDecodeMetaData
                ->getField(
                    ldDecodeMetaData->getFirstFieldNumber(frameNumber + 1))
                .fieldPhaseID;
        qint32 currentPhase2 =
            ldDecodeMetaData
                ->getField(
                    ldDecodeMetaData->getSecondFieldNumber(frameNumber + 1))
                .fieldPhaseID;
        ...
        if (frameNumber < m_numberOfFrames - 1)
          nextPhase1 = ldDecodeMetaData
                           ->getField(ldDecodeMetaData->getFirstFieldNumber(
                               frameNumber + 2))
                           .fieldPhaseID;
        ...
        if (currentPhase2 == expectedIntraPhase) {
          if (lastPhase2 == expectedLastPhase || lastPhase2 == -1) {
            if (nextPhase1 == expectedNextPhase || nextPhase1 == -1) {
              ...
              tbcDebugStream()
                  << "Seq. frame" << m_frames[frameNumber].seqFrameNumber()
                  << "is not in phase sequence with the subsequent frame!";
            }
          } else {
            tbcDebugStream()
                << "Seq. frame" << m_frames[frameNumber].seqFrameNumber()
                << "is not in phase sequence with the preceding frame!";
          }
        } else {
          tbcDebugStream() << "Seq. frame"
                           << m_frames[frameNumber].seqFrameNumber()
                           << "has an incorrect intra-frame phaseID!";
        }
        ...
        if (isPulldown) {
          ...
          if (doubleCheckCounter < 1) {
            if (!m_noStrict) {
              tbcDebugStream()
                  << "Seq. frame" << m_frames[frameNumber].seqFrameNumber()
                  << "looks like a pull-down, but there is no pull-down "
                     "sequence in the surrounding frames - marking as "
                     "false-positive";
              isPulldown = false;
            } else {
              tbcDebugStream()
                  << "Seq. frame" << m_frames[frameNumber].seqFrameNumber()
                  << "looks like a pull-down, but there is no pull-down "
                     "sequence in the surrounding frames"
                  << "- strict checking is disabled, so marking as pulldown "
                     "anyway";
              isPulldown = true;
            }
          }
  ...
  for (qint32 frameNumber = 0; frameNumber < m_numberOfFrames; frameNumber++) {
    ...
    if (frameNumber < m_numberOfFrames - 1) {
      if (vbiData[frameNumber + 1].picNo < vbiData[frameNumber].picNo)
        penaltyPercent = 80.0;
      else
    ...
    qint32 totalDotsInFrame =
        (ldDecodeMetaData->getVideoParameters().fieldHeight * 2) +
        ldDecodeMetaData->getVideoParameters().fieldWidth;
    DropOuts dropOuts1 = ldDecodeMetaData->getFieldDropOuts(
        ldDecodeMetaData->getFirstFieldNumber(frameNumber + 1));
    DropOuts dropOuts2 = ldDecodeMetaData->getFieldDropOuts(
        ldDecodeMetaData->getSecondFieldNumber(frameNumber + 1));
    ...
    double frameDoPercent = 100.0 - (static_cast<double>(frameDoLength) /
                                     static_cast<double>(totalDotsInFrame));

    qint32 syncConfPercent =
        (ldDecodeMetaData
             ->getField(ldDecodeMetaData->getFirstFieldNumber(frameNumber + 1))
             .syncConf +
         ldDecodeMetaData
             ->getField(ldDecodeMetaData->getSecondFieldNumber(frameNumber + 1))
             .syncConf) /
        2;

    // Composite quality score weighted heavily towards dropout coverage
    // (x1000), with secondary contributions from bSNR, sequence continuity
    // penalty, and sync confidence. Normalised to a 0–100 range.
    m_frames[frameNumber].frameQuality((bsnrPercent + penaltyPercent +
                                        static_cast<double>(syncConfPercent) +
                                        (frameDoPercent * 1000.0)) /
                                       1004.0);
  }

  for (qint32 frameNumber = 0; frameNumber < m_numberOfFrames; frameNumber++) {
    m_frames[frameNumber].firstFieldPhase(
        ldDecodeMetaData
            ->getField(ldDecodeMetaData->getFirstFieldNumber(frameNumber + 1))
            .fieldPhaseID);
    m_frames[frameNumber].secondFieldPhase(
        ldDecodeMetaData
            ->getField(ldDecodeMetaData->getSecondFieldNumber(frameNumber + 1))
            .fieldPhaseID);
  }
...
DiscMap::~DiscMap() { delete ldDecodeMetaData; }
...
QDebug operator<<(QDebug dbg, const DiscMap& discMap) {
  dbg.nospace().noquote() << "DiscMap(Frames " << discMap.numberOfFrames()
                          ...
                          << ", detected " << discMap.numberOfPulldowns()
                          << " pulldown frames)";
...
bool DiscMap::isPhaseCorrect(qint32 frameNumber) const {
  ...
  if (frameNumber > 0) {
    ...
    if (m_frames[frameNumber].firstFieldPhase() != expectedNextPhase) {
      tbcDebugStream() << "Frame number" << frameNumber
                       << "phase sequence does not match preceding frame! -"
  ...
  if (frameNumber < m_numberOfFrames - 1) {
    ...
    if (m_frames[frameNumber + 1].firstFieldPhase() != expectedNextPhase) {
      tbcDebugStream() << "Frame number" << frameNumber
                       << "phase sequence does not match following frame! -"
                       << expectedNextPhase << "expected but got"
                       << m_frames[frameNumber].secondFieldPhase();
      return false;
    }
...
bool DiscMap::isPhaseRepeating(qint32 frameNumber) const {
  ...
  if (frameNumber > 0) {
    if ((m_frames[frameNumber].firstFieldPhase() ==
         m_frames[frameNumber - 1].firstFieldPhase()) &&
        (m_frames[frameNumber].secondFieldPhase() ==
         m_frames[frameNumber - 1].secondFieldPhase()))
      return true;
  } else {
...
qint32 DiscMap::getApproximateAudioFieldLength() const {
  return m_audioFieldByteLength / 2;
}
...
qint32 DiscMap::getFirstFieldAudioDataStart(qint32 frameNumber) const {
  ...
  return ldDecodeMetaData->getFieldPcmAudioStart(
      m_frames[frameNumber].firstField());
}
...
qint32 DiscMap::getFirstFieldAudioDataLength(qint32 frameNumber) const {
  ...
  return ldDecodeMetaData->getFieldPcmAudioLength(
      m_frames[frameNumber].firstField());
}
...
qint32 DiscMap::getSecondFieldAudioDataLength(qint32 frameNumber) const {
  if (frameNumber < 0 || frameNumber >= m_numberOfFrames) {
    tbcDebugStream()
        << "getSecondFieldAudioDataLength out of frameNumber range";
    return -1;
  }
  return ldDecodeMetaData->getFieldPcmAudioLength(
      m_frames[frameNumber].secondField());
...
bool DiscMap::saveTargetMetadata(QFileInfo outputFileInfo) {
  ...
  LdDecodeMetaData::VideoParameters sourceVideoParameters =
  ...
  targetMetadata.setPcmAudioParameters(
  ...
  for (qint32 frameNumber = 0; frameNumber < m_numberOfFrames; frameNumber++) {
    if (!m_frames[frameNumber].isPadded()) {
      ...
      LdDecodeMetaData::Field firstSourceMetadata =
          ldDecodeMetaData->getField(firstFieldNumber);
      LdDecodeMetaData::Field secondSourceMetadata =
          ldDecodeMetaData->getField(secondFieldNumber);
      ...
      if (m_isDiscCav) {
        ...
        firstSourceMetadata.vbi.vbiData[1] =
            convertFrameToVbi(m_frames[frameNumber].vbiFrameNumber());
        firstSourceMetadata.vbi.vbiData[2] =
            convertFrameToVbi(m_frames[frameNumber].vbiFrameNumber());

        VbiDecoder::Vbi vbi =
            vbiDecoder.decodeFrame(firstSourceMetadata.vbi.vbiData[0],
                                   firstSourceMetadata.vbi.vbiData[1],
                                   firstSourceMetadata.vbi.vbiData[2],
                                   secondSourceMetadata.vbi.vbiData[0],
                                   secondSourceMetadata.vbi.vbiData[1],
                                   secondSourceMetadata.vbi.vbiData[2]);
        ...
        if (vbi.picNo != m_frames[frameNumber].vbiFrameNumber()) {
          ...
          qInfo() << "Warning: Updated VBI frame number for frame"
                  << m_frames[frameNumber].vbiFrameNumber()
                  << "has been corrupted by existing VBI data - overwriting "
                     "all VBI for frame";
          firstSourceMetadata.vbi.vbiData[0] = 0;
        }
      } else {
        if (!firstSourceMetadata.vbi.inUse)
          firstSourceMetadata.vbi.inUse = true;
        firstSourceMetadata.vbi.vbiData[0] =
            convertFrameToClvPicNo(m_frames[frameNumber].vbiFrameNumber());
        firstSourceMetadata.vbi.vbiData[1] =
            convertFrameToClvTimeCode(m_frames[frameNumber].vbiFrameNumber());
        firstSourceMetadata.vbi.vbiData[2] =
            convertFrameToClvTimeCode(m_frames[frameNumber].vbiFrameNumber());
      }
    ...
    if (frameNumber % notifyInterval == 0)
      qInfo() << "Created metadata for frame" << frameNumber << "of"
              << m_numberOfFrames;
  }
...
qint32 DiscMap::convertFrameToClvPicNo(qint32 frameNumber) {
  LdDecodeMetaData::ClvTimecode timecode =
      ldDecodeMetaData->convertFrameNumberToClvTimecode(frameNumber);
  ...
  QString number = "008" + QString("%1").arg(secondsX1, 1, 16, QChar('0')) +
                   "E" + QString("%1").arg(secondsX3, 1, 10, QChar('0')) +
...
qint32 DiscMap::convertFrameToClvTimeCode(qint32 frameNumber) {
  ...
  QString number = "00F" +
                   QString("%1").arg(timecode.hours, 1, 10, QChar('0')) + "DD" +
