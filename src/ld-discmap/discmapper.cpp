...
bool DiscMapper::process(QFileInfo _inputFileInfo,
                         QFileInfo _inputMetadataFileInfo,
                         QFileInfo _outputFileInfo, bool _reverse,
                         bool _mapOnly, bool _noStrict, bool _deleteUnmappable,
                         bool _noAudio) {
  ...
  qInfo().noquote() << "Processing input metadata for"
                    << inputFileInfo.filePath();
  if (noStrict)
    qInfo() << "Not enforcing strict pulldown checking - this can cause "
               "false-positive detection";
  DiscMap discMap(inputMetadataFileInfo, reverse, noStrict);
  if (!discMap.valid()) {
    qInfo()
        << "Could not process TBC metadata successfully - cannot map this disc";
    return false;
  }
  tbcDebugStream() << discMap;

  qInfo().noquote() << "Input TBC is a" << discMap.discType() << "disc using"
                    << discMap.discFormat();
  ...
  if (!verifyFrameNumberPresence(discMap)) {
    if (mapOnly) {
      ...
      qInfo() << "";
      qInfo()
          << "Unmappable frames detected; deletion requires a TBC video file.";
      qInfo()
          << "Either cancel and re-run with -u (--delete-unmappable-frames),";
      ...
      qInfo() << "Continuing - output metadata is for analysis only and cannot "
                 "be used with a TBC video file.";
    } else if (!deleteUnmappable) {
      qInfo() << "";
      qInfo() << "Disc mapping has failed as there are unmappable frames in "
                 "the disc map!";
      qInfo() << "It is possible that running ld-discmap again with the "
      ...
      qInfo() << "--delete-unmappable-frames is set, so the unmappable frames "
                 "will be deleted";
...
void DiscMapper::removeLeadInOut(DiscMap& discMap) {
  ...
  for (qint32 frameNumber = 0; frameNumber < discMap.numberOfFrames();
       frameNumber++) {
    ...
    if (!discMap.isLeadInOut(frameNumber) && discMap.isDiscCav() &&
        discMap.vbiFrameNumber(frameNumber) == 0) {
      qInfo() << "Warning: Frame with illegal CAV frame number of 0 found... "
                 "Assuming illegal lead in and deleting.";
      discMap.setMarkedForDeletion(frameNumber);
      leadInOutCounter++;
    }
  }
...
void DiscMapper::removeInvalidFramesByPhase(DiscMap& discMap) {
  ...
  for (qint32 frameNumber = 0; frameNumber < discMap.numberOfFrames();
    ...
    if (discMap.getSecondFieldPhase(frameNumber) != expectedNextPhase) {
      if (discMap.vbiFrameNumber(frameNumber) != -1) {
        tbcDebugStream()
            << "Marking frame" << frameNumber << "for deletion (VBI Frame#"
            << discMap.vbiFrameNumber(frameNumber)
            << ") as first and second field phases are not in sequence! -"
            << expectedNextPhase << "expected but got"
            << discMap.getSecondFieldPhase(frameNumber);
      } else {
        tbcDebugStream() << "Marking frame" << frameNumber
                         << "for deletion (VBI Frame# invalid) as first and "
                            "second field phases are not in sequence! -"
                         << expectedNextPhase << "expected but got"
                         << discMap.getSecondFieldPhase(frameNumber);
      }
      discMap.setMarkedForDeletion(frameNumber);
  ...
  qInfo() << "Removing" << removals
          << "frames marked as invalid due to incorrect phase sequence";
  discMap.flush();
}
...
void DiscMapper::correctVbiFrameNumbersUsingSequenceAnalysis(DiscMap& discMap) {
  ...
  for (qint32 frameNumber = 0;
       frameNumber < discMap.numberOfFrames() - scanDistance; frameNumber++) {
    if (!discMap.isPulldown(frameNumber) &&
        discMap.vbiFrameNumber(frameNumber) != -1) {
      ...
      for (qint32 i = 0; i < scanDistance; i++) {
        if (!discMap.isPulldown(frameNumber + i + 1)) {
          if ((discMap.vbiFrameNumber(frameNumber + i + 1) ==
               startOfSequence + expectedIncrement) ||
      ...
      if (count != scanDistance) {
        qint32 check1 = 0;
        for (qint32 i = 0; i < scanDistance; i++) {
          if (vbiGood[i] && !discMap.isPulldown(frameNumber + i + 1))
            check1++;
          else if (!discMap.isPulldown(frameNumber + i + 1))
            break;
        }
        ...
        if (check1 >= 2 && check2 >= 2) {
          ...
          for (qint32 i = 0; i < scanDistance; i++) {
            if (!vbiGood[i]) {
              ...
              if (!discMap.isPulldown(frameNumber + i + 1)) {
                if ((discMap.vbiFrameNumber(frameNumber + i + 1) !=
                     discMap.vbiFrameNumber(frameNumber + i)) &&
                    discMap.isPhaseCorrect(frameNumber + i + 1)) {
                  tbcDebugStream()
                      << "  Position BAD   " << i << "Seq."
                      << discMap.seqFrameNumber(frameNumber + i + 1)
                      << "VBI was"
                      << discMap.vbiFrameNumber(frameNumber + i + 1) << "now"
                      << (startOfSequence + expectedIncrement) << "- Phase"
                      << discMap.getFirstFieldPhase(frameNumber + i + 1) << "/"
                      << discMap.getSecondFieldPhase(frameNumber + i + 1);
                  discMap.setVbiFrameNumber(
                      frameNumber + i + 1, startOfSequence + expectedIncrement);
                  if (!discMap.isPulldown(frameNumber + i + 1))
                    expectedIncrement++;
                  corrections++;
                } else {
                  if (discMap.isPhaseRepeating(frameNumber + i + 1)) {
                    tbcDebugStream()
                        << "  Position REPEAT" << i << "Seq."
                        << discMap.seqFrameNumber(frameNumber + i + 1) << "VBI"
                        << discMap.vbiFrameNumber(frameNumber + i + 1)
                        << "- Phase"
                        << discMap.getFirstFieldPhase(frameNumber + i + 1)
                        << "/"
                        << discMap.getSecondFieldPhase(frameNumber + i + 1);
                    tbcDebugStream()
                        << "  Ignoring sequence break as frame is repeating "
                           "(VBI and phase) rather than out of sequence";
                    if (inError) break;
                  }
                }
              } else {
              ...
              if (!discMap.isPulldown(frameNumber + i + 1))
                tbcDebugStream()
                    << "  Position GOOD  " << i << "Seq."
                    << discMap.seqFrameNumber(frameNumber + i + 1) << "VBI"
                    << discMap.vbiFrameNumber(frameNumber + i + 1) << "- Phase"
                    << discMap.getFirstFieldPhase(frameNumber + i + 1) << "/"
                    << discMap.getSecondFieldPhase(frameNumber + i + 1);
              else
                tbcDebugStream()
...
void DiscMapper::removeDuplicateNumberedFrames(DiscMap& discMap) {
  ...
  tbcDebugStream() << "Building list of VBIs that have more than one entry in "
                      "the discmap...";
  ...
  for (qint32 frameNumber = 0; frameNumber < discMap.numberOfFrames();
    ...
    if (!discMap.isPulldown(frameNumber)) {
      for (qint32 i = frameNumber + 1; i < discMap.numberOfFrames(); i++) {
        if (discMap.vbiFrameNumber(frameNumber) == discMap.vbiFrameNumber(i) &&
            !discMap.isPulldown(i)) {
  ...
  tbcDebugStream()
      << "Sorting the duplicated frame list into numerical order...";
  std::sort(duplicatedFrameList.begin(), duplicatedFrameList.end());
  tbcDebugStream() << "Removing any repeated frame numbers from the duplicated "
                      "frame list...";
  auto last =
      std::unique(duplicatedFrameList.begin(), duplicatedFrameList.end());
  duplicatedFrameList.erase(last, duplicatedFrameList.end());

  tbcDebugStream() << "Found" << duplicatedFrameList.size()
                   << "VBI frame numbers with more than 1 entry in the discmap";

  for (qint32 i = 0; i < duplicatedFrameList.size(); i++) {
    if (duplicatedFrameList[i] != -1) {
      tbcDebugStream() << "VBI Frame number" << duplicatedFrameList[i]
                       << "has duplicates; searching for them...";
      ...
      tbcDebugStream() << "  Found" << discMapDuplicateAddress.size()
                       << "duplicates of VBI frame" << duplicatedFrameList[i];

      qint32 bestDiscMapFrame = discMapDuplicateAddress.first();
      for (qint32 i = 0; i < discMapDuplicateAddress.size(); i++) {
        if (discMap.frameQuality(bestDiscMapFrame) <
            discMap.frameQuality(discMapDuplicateAddress[i])) {
          bestDiscMapFrame = discMapDuplicateAddress[i];
        }
      }

      tbcDebugStream() << "  Highest quality duplicate of VBI"
                       << duplicatedFrameList[i] << "is sequential frame"
                       << discMap.seqFrameNumber(bestDiscMapFrame)
                       << "with a quality of"
                       << discMap.frameQuality(bestDiscMapFrame);
      ...
      if (!discMap.isDiscPal()) {
        qInfo() << "";
        qInfo() << "Warning:";
        qInfo() << "There are frames without a frame number (that are not "
                   "flagged as pulldown) in the duplicate frame list";
        qInfo() << "This probably means that the disc map contains pulldown "
                   "frames that do not follow the normal 1 in 5";
        qInfo() << "pulldown pattern - and disc mapping will likely fail!";
        qInfo() << "";
      } else {
        qInfo() << "";
        qInfo() << "Warning:";
        qInfo() << "There are frames without a frame number in the duplicate "
                   "frame list.  Since numberless frames are";
        qInfo() << "usually unmappable, disc mapping will likely fail unless "
                   "the --delete-unmappable-frames option is";
        qInfo() << "used.";
        qInfo() << "";
      }
    }
  ...
  qInfo() << "Removed" << originalSize - discMap.numberOfFrames()
          << "duplicate frames - disc map size now" << discMap.numberOfFrames()
          << "frames";
}
...
void DiscMapper::numberPulldownFrames(DiscMap& discMap) {
  if (discMap.isDiscCav() && !discMap.isDiscPal()) {
    ...
    for (qint32 i = 1; i < discMap.numberOfFrames(); i++) {
      if (discMap.isPulldown(i))
        discMap.setVbiFrameNumber(i, discMap.vbiFrameNumber(i - 1));
    }

    if (discMap.isPulldown(0)) {
      discMap.setVbiFrameNumber(0, discMap.vbiFrameNumber(1) - 1);
      qInfo() << "Attempted to number pulldown frames, but first frame is a "
                 "pulldown.";
...
bool DiscMapper::verifyFrameNumberPresence(DiscMap& discMap) {
  qInfo() << "Verifying frame numbers are present for all frames in the disc "
             "map...";
  for (qint32 frameNumber = 0; frameNumber < discMap.numberOfFrames();
       frameNumber++) {
    if (discMap.vbiFrameNumber(frameNumber) < 0) {
      qInfo() << "Verification failed - First failed frame was"
              << discMap.seqFrameNumber(frameNumber);
      discMap.debugFrameDetails(frameNumber);
      return false;
    }
  }
...
void DiscMapper::reorderFrames(DiscMap& discMap) {
  ...
  if (discMap.numberOfFrames() > 2) {
    ...
    qint32 initialGap = discMap.vbiFrameNumber(frameNumber + 1) -
                        discMap.vbiFrameNumber(frameNumber);

    if (initialGap > 1000) {
      qInfo() << "Warning: The gap between the first and second VBI number is"
              << initialGap;
      qInfo() << "this is over the 1000 frame threshold, so the first frame "
                 "will be deleted to";
      qInfo() << "avoid generating a big gap.";

      discMap.setMarkedForDeletion(frameNumber);
      discMap.flush();
      qInfo() << "Removed first frame - disc map size now"
              << discMap.numberOfFrames() << "frames";
    }
  }
}
...
void DiscMapper::padDiscMap(DiscMap& discMap) {
  ...
  for (qint32 frameNumber = 0; frameNumber < discMap.numberOfFrames() - 1;
       frameNumber++) {
    if (discMap.vbiFrameNumber(frameNumber) + 1 !=
        discMap.vbiFrameNumber(frameNumber + 1)) {
      if (discMap.isPulldown(frameNumber)) {
        ...
        if (discMap.isPulldown(frameNumber + 1)) {
          if (discMap.vbiFrameNumber(frameNumber) + 1 !=
              discMap.vbiFrameNumber(frameNumber + 2)) {
            if ((discMap.vbiFrameNumber(frameNumber + 2) -
                 discMap.vbiFrameNumber(frameNumber)) != 0) {
              tbcDebugStream()
                  << "Sequence break over pulldown: Current VBI frame is"
                  << discMap.vbiFrameNumber(frameNumber) << "next frame (+1) is"
                  << discMap.vbiFrameNumber(frameNumber + 2) << "gap of"
                  << discMap.vbiFrameNumber(frameNumber + 2) -
                         discMap.vbiFrameNumber(frameNumber)
                  << "frames";

              numberOfGaps++;
              qint32 missingFrames = discMap.vbiFrameNumber(frameNumber + 2) -
                                     discMap.vbiFrameNumber(frameNumber);
              ...
              if ((discMap.vbiFrameNumber(frameNumber + 2) -
                   discMap.vbiFrameNumber(frameNumber)) == 0) {
                qInfo() << "Warning: zero-length gap detected across a "
                           "pulldown boundary - this frame";
                qInfo() << "will be skipped. If this causes problems, re-run "
          ...
          if (!discMap.isClvOffset(frameNumber)) {
            qint32 gapLength = discMap.vbiFrameNumber(frameNumber + 1) -
                               discMap.vbiFrameNumber(frameNumber) - 1;
            tbcDebugStream()
                << "Sequence break: Current VBI frame is"
                << discMap.vbiFrameNumber(frameNumber) << "next frame is"
                << discMap.vbiFrameNumber(frameNumber + 1) << "gap of"
                << gapLength << "frames";

            if (gapLength > 1000) {
              qInfo() << "Warning: Detected a sequence break between VBI frame"
                      << discMap.vbiFrameNumber(frameNumber) << "and"
                      << "VBI frame" << discMap.vbiFrameNumber(frameNumber + 1)
                      << "representing";
              qInfo() << "a gap of" << gapLength
                      << "frames.  This is over the threshold of 1000 frames "
                         "and could indicate that mapping";
              qInfo() << "has failed due to badly corrupted VBI frame number "
                         "data in the source TBC file.";
            }

            numberOfGaps++;
            qint32 missingFrames = discMap.vbiFrameNumber(frameNumber + 1) -
                                   discMap.vbiFrameNumber(frameNumber) - 1;
            totalMissingFrames += missingFrames;
            startFrame.append(frameNumber);
            paddingLength.append(missingFrames);
  ...
  if (numberOfGaps > 0) {
    qInfo() << "Found" << numberOfGaps << "gaps representing"
            << totalMissingFrames << "missing frames in the disc map";
    qInfo() << "Note: The disc map has been padded.  This means that there "
               "were missing frames";
    qInfo() << "that will be represented by black frames in the output video.";
  } else
    qInfo() << "No gaps found in the disc map";

  if (clvOffsetFrames > 0)
    qInfo() << "There were" << clvOffsetFrames
            << "CLV timecode offsets in the disc map";
  qInfo() << "After padding the disc map contains" << discMap.numberOfFrames()
          << "frames";
}
...
void DiscMapper::rewriteFrameNumbers(DiscMap& discMap) {
  if (discMap.isDiscCav() && !discMap.isDiscPal()) {
    ...
    if (present)
      qInfo() << "Search complete; pulldown frames are present.  Renumbering "
                 "disc map...";
    else {
...
void DiscMapper::deleteUnmappableFrames(DiscMap& discMap) {
  ...
  for (qint32 frameNumber = 0; frameNumber < discMap.numberOfFrames();
       frameNumber++) {
    if (discMap.vbiFrameNumber(frameNumber) < 0 &&
        !discMap.isPulldown(frameNumber)) {
      tbcDebugStream() << "Marking frame" << frameNumber
                       << "for deletion (unmappable)";
      discMap.setMarkedForDeletion(frameNumber);
    }
  }
  discMap.flush();
...
bool DiscMapper::saveDiscMap(DiscMap& discMap) {
  ...
  if (!noAudio) {
    if (!sourceAudio.open(inputFileInfo)) {
      qInfo() << "Cannot open source audio file:"
              << inputFileInfo.absolutePath() + "/" +
                     inputFileInfo.completeBaseName() + ".pcm";
      sourceVideo.close();
      sourceAudio.close();
      return false;
    }

    targetAudio.setFileName(outputFileInfo.absolutePath() + "/" +
                            outputFileInfo.completeBaseName() + ".pcm");
    if (targetAudio.exists()) {
      qInfo() << "Target audio file already exists:" << targetAudio.fileName()
              << "- Cannot proceed!";
      sourceVideo.close();
      sourceAudio.close();
      return false;
  ...
  for (qint32 frameNumber = 0; frameNumber < discMap.numberOfFrames();
    ...
    if (!discMap.isPadded(frameNumber)) {
      ...
      if (firstFieldNumber < secondFieldNumber) {
        if (!targetVideo.write(
                reinterpret_cast<const char*>(sourceFirstField.data()),
                sourceFirstField.size() * 2))
          writeFail = true;
        if (!targetVideo.write(
                reinterpret_cast<const char*>(sourceSecondField.data()),
                sourceSecondField.size() * 2))
          writeFail = true;
      } else {
        if (!targetVideo.write(
                reinterpret_cast<const char*>(sourceSecondField.data()),
      ...
      if (!noAudio) {
        if ((discMap.getFirstFieldAudioDataLength(frameNumber) > 0) &&
          ...
          sourceAudioFirstField = sourceAudio.getAudioData(
              discMap.getFirstFieldAudioDataStart(frameNumber),
              discMap.getFirstFieldAudioDataLength(frameNumber));
          sourceAudioSecondField = sourceAudio.getAudioData(
              discMap.getSecondFieldAudioDataStart(frameNumber),
              discMap.getSecondFieldAudioDataLength(frameNumber));

          if (!targetAudio.write(
                  reinterpret_cast<const char*>(sourceAudioFirstField.data()),
                  sourceAudioFirstField.size() * 2))
            writeFail = true;
          if (!targetAudio.write(
                  reinterpret_cast<const char*>(sourceAudioSecondField.data()),
                  sourceAudioSecondField.size() * 2))
            writeFail = true;
        } else {
          if (discMap.getFirstFieldAudioDataLength(frameNumber) < 1) {
            qInfo() << "Warning: Input file seems to have zero audio data in "
                       "the first field of frame number #"
                    << frameNumber;
            qInfo() << "The audio output might be corrupt.";
          }
          if (discMap.getSecondFieldAudioDataLength(frameNumber) < 1) {
            qInfo() << "Warning: Input file seems to have zero audio data in "
                       "the second field of frame number #"
                    << frameNumber;
            qInfo() << "The audio output might be corrupt.";
          }
        }
      ...
      if (!targetVideo.write(
              reinterpret_cast<const char*>(missingFieldData.data()),
              missingFieldData.size() * 2))
        writeFail = true;
      if (!targetVideo.write(
              reinterpret_cast<const char*>(missingFieldData.data()),
              missingFieldData.size() * 2))
      ...
      if (!noAudio) {
        if (!targetAudio.write(
                reinterpret_cast<const char*>(missingFieldAudioData.data()),
                missingFieldAudioData.size() * 2))
          writeFail = true;
        if (!targetAudio.write(
                reinterpret_cast<const char*>(missingFieldAudioData.data()),
                missingFieldAudioData.size() * 2))
    ...
    if (frameNumber % notifyInterval == 0)
      qInfo() << "Written frame" << frameNumber << "of"
              << discMap.numberOfFrames();

    if (writeFail) {
      qWarning()
          << "Writing fields to the target TBC file failed on frame number"
          << frameNumber;
      targetVideo.close();
      sourceVideo.close();
      return false;
