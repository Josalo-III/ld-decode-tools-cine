 ...
 * File:        segmenter.cpp
 * Module:      segmentation
 * Purpose:     Phase-discontinuity boundary marking and solver-flag clearing
 *
...
int segmentDisc(CineDisc& disc) {
  ...
  qInfo() << "segmenter::segmentDisc: marked" << boundaryCount
          << "boundary(s).";
...
// Clears all solver-owned flags AND manual edit vetoes. Never called
// implicitly. Covers: isEditBoundary, isEditVetoed, cadenceId,
// cadenceIndexPresumed, pulldownRole. This is the only path that discards a
// user veto, which is why it is a separate, explicitly confirmed command from
// clearEditBoundaries().
