...
ProbeResult probe(CineDisc& disc) {
  ...
  for (int fi = 0; fi < nFrames; ++fi) {
    ...
    const auto v = vbiDecoder.decodeFrame(vbi1[0], vbi1[1], vbi1[2], vbi2[0],
                                          vbi2[1], vbi2[2]);
    ...
    fv.isClv =
        (v.clvHr != -1 && v.clvMin != -1 && v.clvSec != -1 && v.clvPicNo != -1);
  ...
  qInfo() << "vbiProbe::probe: scanned" << nFrames << "frame(s)"
          << "| CAV hits =" << cavCount << "| CLV hits =" << clvCount;

  if (cavCount == 0 && clvCount == 0) {
    qCritical() << "vbiProbe::probe: no CAV or CLV VBI detected in" << nFrames
                << "frame(s) - cannot determine disc type."
