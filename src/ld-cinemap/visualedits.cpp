...
/*************************************************************
 ...
 Visual edit detection for ld-cinemap
...
// see line 405 for a thorough description of the different weights
						   ...
						   double peakFactor)
	{
		...
		auto isRampContext = [&](int idx) -> bool {
			...
			if (LOG_RAMP_VETO && ramp) {
				tbcDebugStream().nospace() << "PA: EditDetector: ramp veto at field "
		...
		for (int i = 5; i <= totalFields - 2; ++i) {
			...
			if (LOG_PROGRESS && hb.elapsed() >= 1000) {
				...
				qInfo().noquote()
					<< QString("PA: Visual edit scan %1/%2 (%3%) cache=%4 edits=%5")
			...
			if (ramp) {
				if (LOG_RAMP_VETO && (LOG_VERBOSE_REJECT || LOG_CANDIDATES || dbg)) {
					qInfo().nospace() << "PA: EditDetector: ramp veto commit@field " << i
									  << " reason=" << reason;
				}
				continue;
			}
			...
			if (continuityVeto) {
				if (LOG_VERBOSE_REJECT || LOG_CANDIDATES) {
					qInfo().nospace()
						<< "PA: EditDetector: continuity veto at field " << i
						<< " reason="       << reason
						<< " corr="         << corr
						<< " strong="       << ds.strongCells
						<< " total="        << ds.total
