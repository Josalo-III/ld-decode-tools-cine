...
struct ProbeResult {
    ...
    // Convenience: returns picNo for a 0-based frame index, or -1 if out of range.
    ...
    bool isExcluded(int frameIdx) const {
        if (frameIdx < 0 || frameIdx >= static_cast<int>(frames.size())) return true;
