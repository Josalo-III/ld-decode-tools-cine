 ...
 * File:        cinedisc.cpp
 * Module:      disc
 * Purpose:     LdDecodeMetaData-backed CineDisc and edit-list parsing
 *
...
// Parse seqNo keys from comma-separated string, optionally supporting ranges
// (e.g., "10-20")
static std::vector<qint32> parseSeqNoKeys(const QString& text,
                                          bool allowRanges) {
...
static bool parseCadenceOverrideSpec(const QString& text, qint32& firstField,
                                     qint32& lastField, int& firstCid) {
  ...
  if (firstCid == CADENCE_UNKNOWN || firstCid == CADENCE_VIDEO ||
...
static int cadenceIdAtOffset(int firstCid, int offset) {
  ...
  const int base =
      cadenceIsInverted(firstCid) ? CADENCE_NTSC_INVERTED_OFFSET : 0;
...
class CineDiscMeta : public CineDisc {
 public:
  ...
  static std::unique_ptr<CineDiscMeta> load(const QString& tbcPath,
    ...
    if (nFrames < 2) {
      qCritical() << "CineDiscMeta: metadata contains only" << nFrames
                  << "frame(s) — too small to process.";
      return nullptr;
    }
    if (nFrames > 108000) {
      qCritical() << "CineDiscMeta: metadata contains" << nFrames
                  << "frames — exceeds maximum supported size.";
      return nullptr;
    }
  ...
  LdDecodeMetaData& getMetaData() override { return *m_md; }

  const LdDecodeMetaData& getMetaData() const override { return *m_md; }

  const QString& getTbcPath() const override { return m_tbcPath; }

  int getVideoFieldLength() const override { return m_videoFieldLength; }

  bool isDiscPal() const override { return m_isDiscPal; }

  bool isDiscCav() const override { return m_isDiscCav; }

  void setIsDiscCav(bool cav) override { m_isDiscCav = cav; }

  int getNumberOfFrames() const override { return m_numberOfFrames; }
  ...
  bool getReverseFieldOrder() const override { return m_reverseFieldOrder; }

  QString getFilename() const override { return m_tbcPath; }
  ...
  int applyEditBlacklistSeqNoKeys(
      const QString& csvSeqNoListWithRanges) override {
    const auto keys =
        parseSeqNoKeys(csvSeqNoListWithRanges, /*allowRanges=*/true);
  ...
  int applyCadenceOverrideFieldRange(const QString& rangeSpec) override {
    ...
    for (qint32 idx = firstField; idx <= lastField && idx <= totalFields;
         ++idx) {
 ...
 private:
  ...
  int applyEditOverridesBySeqNoKeys(const std::vector<qint32>& seqNoKeys,
                                    bool value) {
    ...
    for (qint32 idx = 1; idx <= totalFields; ++idx) {
      ...
      auto modifiedField = field;
      if (value)
        modifiedField.cinemap.forceEditBoundary();
      else
        modifiedField.cinemap.vetoEditBoundary();
      m_md->updateField(modifiedField, idx);
      changed++;
    }
